...
| **`brain-managed-memory-boundary`** | Replaces Brain's flat raw-SQLite payload matcher with its existing structural command parser, refuses direct `sqlite3` access to exact managed AgentDB stores even when Brain retrieval is switched off, and adds one audited read-only MCP diagnostic that returns only an exact-key row count. It patches executable hook/MCP bytes after native activation, without selecting a version or touching `active.json`, updater state, or KB data | [stuinfla/ruvnet-brain#102](https://github.com/stuinfla/ruvnet-brain/issues/102) · [#103](https://github.com/stuinfla/ruvnet-brain/issues/103) · follow-up to [#48](https://github.com/stuinfla/ruvnet-brain/issues/48#issuecomment-5169487947) |
...
| [Brain #102](https://github.com/stuinfla/ruvnet-brain/issues/102) | **Open; reproduced on active Brain 4.0.7.** The flat payload matcher misses valid `sqlite3 -json` / `-readonly` invocations and reports prose, searches, and comments as raw database access | `brain-managed-memory-boundary` removes only that flat matcher and reuses Brain's structural invocation parser. Native Categories 1, 2, and 3 remain unchanged |
| [Brain #103](https://github.com/stuinfla/ruvnet-brain/issues/103) | **Open; enforcement absent on active Brain 4.0.7.** Managed AgentDB stores can still be opened through raw `sqlite3`; the existing hook is advisory and disables its whole response with Brain retrieval | Keep `brain-managed-memory-boundary` until a native guard refuses direct managed-store access and exposes a narrow audited diagnostic interface |
