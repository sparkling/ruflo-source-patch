...
startup. Brain 4.0.36 also exposes a live-update edge tracked in
[#128](https://github.com/stuinfla/ruvnet-brain/issues/128): fresh sessions must discover only the
registry generation, while an already-open session must retain a readable catalog path until its
lease/grace period ends. Immediate pruning satisfies only the first half and can strand the second.
...
| [Brain #129](https://github.com/stuinfla/ruvnet-brain/issues/129) | **Open on published 4.0.36.** Linux `--enable-nightly` exits successfully after printing instructions, and its cron recipe invokes only `forge-update.mjs --apply`, bypassing host convergence | Keep the one native SessionStart updater; do not add a downstream scheduler. Upstream should install/verify a user systemd timer with an owned cron fallback that invokes the existing full host coordinator |
| [Brain #130](https://github.com/stuinfla/ruvnet-brain/issues/130) | **Open on published 4.0.36.** One failed 27-store update retained 27 full snapshots (~52 GB), then native `reclaimBackups()` refused all of them because an ordinary `node_modules/.bin/semver` symlink made store inventory “incomplete” | No raw deletion or replacement reclaimer. Preserve the backups until upstream scopes inventory to governed stores, retains at most one transactional rollback, and exposes a supported dry-run reclaim command |
| [Brain #133](https://github.com/stuinfla/ruvnet-brain/issues/133) | **Open on published 4.0.36.** #122 intentionally exits an idle worker after 15 minutes, but the stable parent owns a separate 30-minute timer and records the child's clean retirement as an unexpected `worker-exit`; doctor and SessionStart then report false degradation until the next search respawns it | No second downstream lifecycle owner. Upstream should let the stable parent own the 15-minute retirement, persist an explicit healthy-idle receipt, prove next-search respawn, and keep real crashes fail-closed |
