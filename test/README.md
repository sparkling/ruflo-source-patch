...
| `ruflo-instruction-contract.mjs` | Ruflo #3153's generated instruction contract: authenticated nested/standalone Claude/Codex bundles, every root template, platform and packaged task skills, MCP-first runtime examples, valid schemas, enterprise evidence labels, exact anchors, skill mutation proof, DB/WAL non-interference, public CLI lifecycle, and byte-exact uninstall. |
| `ruflo-instruction-migration.mjs` | Ruflo #3153's existing-project migration: exact root-block and built-in-skill revision attribution, shared-policy/Claude-overlay/custom-root preservation, allowlisted legacy skill upgrade, full-fleet preflight, symlink refusal, CRLF/mode retention, idempotence, and proof that databases/WAL/runtime processes are never touched. |
