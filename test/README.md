...
| `ruflo-instruction-contract.mjs` | Ruflo #3153's generated instruction contract: authenticated nested/standalone Claude/Codex bundles, every root template, platform and packaged task skills, MCP-first runtime examples, explicit managed `ruflo` executable selection, prior-patch upgrade from proven pristine, exact v1/current platform recovery from a poisoned backup, valid schemas, DB/WAL non-interference, public CLI lifecycle, and byte-exact uninstall. |
| `ruflo-instruction-migration.mjs` | Ruflo #3153's existing-project migration: exact root-block and built-in-skill revision attribution, contract-v1 to v2 executable-selection upgrade inside both generated and customized sections, shared-policy/Claude-overlay/custom-root preservation, allowlisted legacy skill upgrade, full-fleet preflight, symlink refusal, CRLF/mode retention, idempotence, and proof that databases/WAL/runtime processes are never touched. |
| `ruflo-model-contract.mjs` | Ruflo #3215's exact-model contract: bounded native model IDs, caller-owned allocation, unchanged three-tier routing, shared-file composition, exact same-version pristine recovery, drift refusal, terminal retirement, and byte-exact uninstall. |
