...
function transformed(spec, pristine, file) {
  ...
  const editCounts = Array.isArray(spec.edits) ? spec.edits : [spec.edits];
  if (result.missing.length || !editCounts.includes(result.applied.length)) {
    throw new Error(`${file}: exact patch anchors incomplete (${result.missing.join(', ') || `${result.applied.length}/${editCounts.join('|')}`})`);
  }
