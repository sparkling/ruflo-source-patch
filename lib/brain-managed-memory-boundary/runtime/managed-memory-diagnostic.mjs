...
function runSqlite(databaseUri, sql, env) {
  return new Promise((resolve) => {
    const child = spawn('sqlite3', ['-batch', '-readonly', '-json', databaseUri], {
...
export async function callManagedMemoryDiagnostic(args, env = process.env) {
  try {
    ...
    if (before[0].absent) return failure('managed AgentDB store does not exist');
    if (!before[1].absent || !before[2].absent) {
      return failure('managed AgentDB store has live WAL/SHM sidecars; retry after the owning writer closes');
    }
    const databaseUri = pathToFileURL(store.path);
    databaseUri.searchParams.set('mode', 'ro');
    // A checkpointed WAL-mode database with no sidecars cannot prepare a normal SQLite -readonly
    // query. `immutable=1` prevents SQLite from creating sidecars; the identity-rich snapshots
    // above and below reject a concurrent whole-image replacement or in-place change.
