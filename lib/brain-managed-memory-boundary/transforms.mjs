...
export function patchHijack(source) {
  return edits(source, [
    ['managed-gate', HIJACK_INPUT, HIJACK_GATE],
    ['remove-flat-category-four', CATEGORY_FOUR, ''],
  ]);
}

export function patchShim(source) {
  return edits(source, [['off-contract', SHIM_ENTRY, SHIM_PATCHED]]);
}
...
export function patchMcp(source) {
  return edits(source, [
    ...
    ['diagnostic-dispatch', MCP_DISPATCH, MCP_DISPATCH_PATCHED],
  ]);
}

export const VENDOR_SPECS = Object.freeze([
  { id: 'hijack', relative: 'scripts/hijack-ruvnet.sh', patch: patchHijack, edits: 2 },
