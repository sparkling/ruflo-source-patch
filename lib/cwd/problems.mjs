...
// A RETIREMENT IS NOT A PROBLEM, and it must never be reported as one. It is the good ending: upstream
// now does this job, we proved the replacement is present and runnable HERE, and we stood down.
//
// It is deliberately kept out of isProblem(). The problems banner opens with "ATTENTION — a patch may no
// longer be doing anything", which is exactly the wrong thing to say about a patch that is no longer
// NEEDED. Crying wolf over good news is how a user learns to ignore the banner that matters.
//
// (`error retiring …` does NOT match this and DOES match isProblem, via `error `. A retirement that only
// half-happened is a real failure and gets the loud channel.)
export const isRetirement = (l) => /^retired /.test(l);
...
/**
 * Merge NEW problem lines into whatever is already recorded, rather than replacing it —
 * recordProblems() overwrites, which is right for "here is the complete current problem set" (what
 * runOnce() computes every tick) but wrong for "something urgent just happened, on top of whatever
 * runOnce already found this tick" (a stale MCP client needing controlled host reconnection). Calling
 * recordProblems() a second time with only the new lines would silently drop runOnce's own findings
 * from problems.json. De-duplicates, and — because the merged signature differs from whatever was
 * last shown whenever `extra` contains anything genuinely new — recordProblems's own shownAt logic
 * makes a first-time addition announce immediately, never waiting out the 30-minute rate limit.
 */
export function addProblems(extra) {
  if (!extra || !extra.length) return;
  let existing = [];
  try { existing = JSON.parse(fs.readFileSync(PROBLEMS_PATH, 'utf8')).problems || []; } catch { /* none yet */ }
  recordProblems([...new Set([...existing, ...extra])]);
}
