...
import { STATE_PATH, STABLE_DIR, GLOBAL_ROOTS, verifiedGlobalRoot } from './paths.mjs';
...
export function readState() {
  try {
    ...
    return {
      ...
      globalRoots: Array.isArray(s.globalRoots)
        ? s.globalRoots.map(verifiedGlobalRoot).filter(Boolean)
        : [],
    ...
    if (err?.code === 'ENOENT') {
      return { patchTargets: [], pluginTargets: [], retired: {}, globalRoots: [], all: false };
    }
...
export function writeState(state) {
  ...
  const clean = {
    ...
    // Capture roots while the foreground shell still has its rich PATH. Later
    // SessionStart/cron runs validate and reuse them even under a narrow PATH.
    globalRoots: [...new Set([...(state.globalRoots || []), ...GLOBAL_ROOTS])]
      .map(verifiedGlobalRoot).filter(Boolean),
...
export function addTargets(targets) {
  return withLock(() => {
    const cur = readState();
    const live = targets.filter((target) => !cur.retired[target]);
    return writeState({ ...cur, patchTargets: [...new Set([...cur.patchTargets, ...live])] });
  });
}
...
export function addPluginTargets(targets) {
  return withLock(() => {
    const cur = readState();
    const live = targets.filter((target) => !cur.retired[target]);
    return writeState({ ...cur, pluginTargets: [...new Set([...cur.pluginTargets, ...live])] });
  });
}
