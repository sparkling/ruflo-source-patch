...
export function readState() {
  try {
    ...
    return {
      ...
      // The user's veto. Someone may deliberately want ours (pinned to an old CLI, or they judge
      // upstream's fix worse). Auto-uninstalling out from under them is hostile.
      pinned: Array.isArray(s.pinned) ? s.pinned.filter(KNOWN) : [],
    };
  } catch {
    return { patchTargets: [], pluginTargets: [], retired: {}, pinned: [] };
  }
}
...
export function writeState(state) {
  ...
  const clean = {
    ...
    pinned: (state.pinned || []).filter(KNOWN),
  };
...
/** Record a target as retired and drop it from the installed sets. Terminal until `unretire`. */
...
export function unretire(target) {
  return withLock(() => {
    const cur = readState();
    const retired = { ...cur.retired };
    delete retired[target];
    return writeState({ ...cur, retired });
  });
}

export const isRetired = (target, state = readState()) => Boolean(state.retired[target]);

/** The user's veto: never auto-retire this one. */
export function pinTarget(target) {
  return withLock(() => {
    const cur = readState();
    return writeState({ ...cur, pinned: [...new Set([...cur.pinned, target])] });
  });
}

export function unpinTarget(target) {
  return withLock(() => {
    const cur = readState();
    return writeState({ ...cur, pinned: cur.pinned.filter((t) => t !== target) });
  });
}
