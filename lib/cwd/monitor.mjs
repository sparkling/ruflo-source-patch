// Target: monitor — keep the patches live between sessions.
//
// WHY it exists: the SessionStart hook only fires when a Claude Code session STARTS.
// `npx -y ruflo@latest` fetches a NEW cache dir the moment a version changes, and a
// `ruflo update` can land mid-session — so a fresh, UNPATCHED copy can be running for
// hours while the hook sits idle until the next restart. The monitor closes that window.
//
// HOW: no resident process, no daemon. This project exists partly because ruflo daemons
// multiply; shipping another long-lived watcher would be poor taste. Instead the OS
// scheduler (launchd on macOS, cron on Linux) runs a SHORT-LIVED check on an interval.
// The check re-applies the installed target set and exits. It writes only when bytes
// actually differ, so a steady-state tick is a few stats and no I/O.
//
// Actions: install | uninstall | status | run | check
//   install    register the scheduled job (default every 5 min; RSP_MONITOR_INTERVAL)
//   uninstall  remove it
//   status     is it scheduled? when did it last run? is anything drifting right now?
//   run        the job body (re-apply + log) — what the scheduler invokes
//   check      dry-run: report drift, exit 1 if any file is not in its expected state

import fs from 'node:fs';
import path from 'node:path';
import { execFileSync } from 'node:child_process';
import { apply, inspect, scanUncoveredBuilds } from './patch-library.mjs';
import { applyPlugins, inspectPlugins } from '../plugin-registry.mjs';
import { readState, isEmpty, withPatchMutationLock } from './state.mjs';
import { recordProblems, isProblem, isFailure } from './problems.mjs';
import { beat, monitorHealthProblems } from './health.mjs';
import { retireSuperseded } from '../supersede.mjs';
import { stableLibDrift, stableLibSourceKind, healStableLib } from './stable.mjs';
import { MONITOR_META_PATH, HEARTBEAT_PATH } from './paths.mjs';
import { STABLE_DIR, STABLE_LIB, HOME_BASE } from './paths.mjs';

export const MONITOR_LABEL = 'com.sparkleideas.ruflo-source-patch.monitor';
export const MONITOR_LOG = path.join(STABLE_DIR, 'monitor.log');
// launchd/cron give a failing job NOWHERE to write its crash — a node that dies before
// monitor-run.mjs's try/catch (a bad interpreter, a module load error) vanishes without a trace,
// which is exactly how a dead watchdog stays invisible. The plist redirects the job's stderr+stdout
// here so a launch failure is CAPTURED, and the prompt hook surfaces it on recovery (ADR-021).
export const MONITOR_STDERR = path.join(STABLE_DIR, 'monitor-stderr.log');
// Mirrors the repo layout — see paths.mjs. Re-running `monitor install` rewrites the
// scheduler entry, so a move here self-heals on the next install.
const MONITOR_SCRIPT = path.join(STABLE_LIB, 'cwd', 'monitor-run.mjs');
// HOME_BASE, not os.homedir(): otherwise a sandboxed/test install would write a real
// LaunchAgent into the user's actual ~/Library/LaunchAgents.
const PLIST = path.join(HOME_BASE, 'Library', 'LaunchAgents', `${MONITOR_LABEL}.plist`);
const CRON_TAG = '# ruflo-source-patch monitor';

const DEFAULT_INTERVAL = 300; // seconds
export const interval = () => {
  const n = parseInt(process.env.RSP_MONITOR_INTERVAL || '', 10);
  return Number.isFinite(n) && n >= 30 ? n : DEFAULT_INTERVAL;
};

// ─── the job body ────────────────────────────────────────────────────────────

export function appendLog(line) {
  try {
    fs.mkdirSync(STABLE_DIR, { recursive: true });
    // Keep it small — this runs on a timer forever.
    try {
      if (fs.statSync(MONITOR_LOG).size > 256 * 1024) {
        const tail = fs.readFileSync(MONITOR_LOG, 'utf8').split('\n').slice(-200).join('\n');
        fs.writeFileSync(MONITOR_LOG, tail);
      }
    } catch { /* no log yet */ }
    fs.appendFileSync(MONITOR_LOG, `${new Date().toISOString()} ${line}\n`);
  } catch { /* best-effort */ }
}

/** Re-apply the installed set. Returns what it had to fix (empty = steady state). */
function runOnceUnlocked() {
  // Proof of life FIRST, before any work that could throw. A tick that dies halfway still
  // proves the scheduler is firing — and conflating "the monitor is dead" with "the monitor
  // hit an error" would send you looking in the wrong place.
  beat();

  let state = readState();
  if (isEmpty(state)) return { skipped: true, repaired: 0 };

  // RETIRE BEFORE REPAIRING, or this same tick would faithfully re-apply the target it is about to
  // stand down. A target retires only when its replacement is proven PRESENT and RUNNABLE on this
  // machine (see supersede.mjs) — never because an issue was closed, which this week was wrong twice.
  const sup = retireSuperseded(state);
  if (sup.retired > 0 || sup.log.length) {
    for (const l of sup.log) appendLog(`RETIRED ${l}`);
    state = readState();                                   // the retired target is gone from state now
    if (isEmpty(state)) return { skipped: true, repaired: 0, retired: sup.retired };
  }

  // Are WE the code the user installed? The stable copy under ~/.ruflo-source-patch/lib is what
  // this process is running, and until now nothing ever refreshed it after the first install —
  // so a package upgrade changed nothing about what the hook and this monitor actually do, in
  // total silence. Heal from the packaged modules; it lands on the next tick. (See stable.mjs;
  // returns null under npx-only usage, where there is no durable package to compare against —
  // unknown, which we do not dress up as healthy.)
  const drifted = stableLibDrift();
  if (drifted && drifted.length) {
    const { healed } = healStableLib();
    appendLog(`SELF-UPDATE stable copy was ${drifted.length} module(s) behind the installed package (${drifted.slice(0, 4).join(', ')}${drifted.length > 4 ? ', …' : ''}) — refreshed ${healed}; effective next tick`);
  }

  const cli = apply(state.patchTargets);
  if (cli.patched > 0) {
    // Something overwrote a patched file (npx fetched a new copy, or ruflo updated).
    appendLog(`REPAIRED ${cli.patched} file(s) [${state.patchTargets.join(',')}] — ${cli.log.filter((l) => l.startsWith('patched')).join(' | ')}`);
  }
  // A re-baseline means the VENDOR file changed under us and we adopted it as the new
  // pristine. Never let that pass silently: it is the moment to check the anchors still
  // hold, and it would otherwise be invisible (the repair line above looks routine).
  for (const l of cli.log.filter((l) => l.startsWith('re-baselined'))) appendLog(`REBASELINE ${l}`);

  // The plugin half. A `/plugin update` replaces ruflo-adr wholesale and drops these
  // patches silently — and a reverted adr-index patch doesn't fail loudly, it just goes
  // back to reporting success while writing nothing.
  const plug = applyPlugins(state.pluginTargets);
  if (plug.patched > 0) {
    appendLog(`REPAIRED ${plug.patched} plugin file(s) [${state.pluginTargets.join(',')}] — ${plug.log.filter((l) => !l.includes('re-baselined')).join(' | ')}`);
  }
  for (const l of plug.log.filter((l) => l.includes('re-baselined'))) appendLog(`REBASELINE ${l}`);
  if (plug.incomplete > 0) {
    appendLog(`WARN ${plug.incomplete} plugin file(s) only PARTIALLY patched — upstream shape changed; run \`adr-index status\``);
  }

  // An anchor that no longer matches means the patch is doing NOTHING to that file, while
  // `status` still says the target is installed. Previously these lines were dropped —
  // runOnce only logged entries starting with 'patched' — so the one failure that most
  // needs a human went unrecorded even here.
  // isFailure() is the SHARED predicate (problems.mjs), not a local regex: the local copy
  // missed `error …` lines, which are the ones we can least afford to drop.
  for (const l of [...cli.log, ...plug.log].filter(isFailure)) appendLog(`WARN ${l}`);

  const uncovered = scanUncoveredBuilds();
  for (const w of uncovered) appendLog(`WARN ${w}`);

  // Leave a note the notifier can pick up. The monitor is a detached job — it cannot reach
  // your session, only leave something behind. A line in monitor.log is a note nobody
  // reads; this one is surfaced on your next prompt.
  //
  // The uncovered-build scan goes in TOO. It exists because 38 daemons accumulated from a
  // package no entry covered, while `daemon status --all` reported health — and until now its
  // finding went ONLY to monitor.log, i.e. to the note nobody reads. A detector whose output
  // no one sees is not a detector.
  recordProblems([
    ...[...cli.log, ...plug.log].filter(isProblem),
    ...uncovered,
  ]);

  const repaired = cli.patched + plug.patched;
  return {
    skipped: false,
    repaired,
    unchanged: cli.unchanged + plug.unchanged,
    incomplete: (cli.incomplete || 0) + (plug.incomplete || 0),
    errors: (cli.errors || 0) + (plug.errors || 0),
    result: { cli, plug },
  };
}

export function runOnce() {
  return withPatchMutationLock(runOnceUnlocked);
}

// ─── drift check (no writes) ─────────────────────────────────────────────────

export function checkDrift() {
  const state = readState();
  const found = inspect();
  // Inspect only desired plugin targets. Some plugin status checks query the Codex
  // registry; doing that for every uninstalled target turns a cheap monitor check into
  // needless subprocess work and lets unrelated host availability affect drift.
  const plug = inspectPlugins(state.pluginTargets);
  const drifting = [];
  for (const t of state.patchTargets) {
    const f = found[t];
    if (f.files === 0) drifting.push(`${t}: no installed target files discovered`);
    else if (f.satisfied < f.files) {
      drifting.push(`${t}: ${f.files - f.satisfied}/${f.files} file(s) UNSATISFIED (${f.patched} patched, ${f.native} native)`);
    }
  }
  for (const t of state.pluginTargets) {
    const f = plug[t];
    if (f.files === 0) drifting.push(`${t}: no installed plugin files discovered`);
    else if (f.patched < f.files) drifting.push(`${t}: ${f.files - f.patched}/${f.files} plugin file(s) UNPATCHED`);
  }
  // Stale stable copy = the hook and the monitor are running code the user no longer has
  // installed. It is not a patch drifting; it is the PATCHER drifting, so it is reported
  // separately rather than folded into `drifting`. null means automatic comparison is unavailable;
  // stableLibSourceKind distinguishes a deliberate mutable-checkout boundary from missing provenance.
  const staleLib = stableLibDrift();

  return {
    installed: [...state.patchTargets, ...state.pluginTargets],
    drifting,
    staleLib,
    stableLibSource: stableLibSourceKind(),
    uncovered: scanUncoveredBuilds(),
  };
}

// ─── scheduling ──────────────────────────────────────────────────────────────

// The node the scheduler should invoke — a VERSION-STABLE one wherever possible.
//
// THE BUG THIS FIXES (ADR-021). `process.execPath` under a version manager is pinned to ONE version:
// mise `.../mise/installs/node/24.14.1/bin/node`, asdf `.../.asdf/installs/nodejs/<v>/bin/node`, volta
// `.../.volta/tools/image/node/<v>/bin/node`. The next `node upgrade` (or a GC of the old version)
// deletes that exact path, and launchd/cron are left invoking a program that no longer exists — the
// monitor dies silently until a session-start heal. Every one of these managers also ships a
// VERSION-STABLE launcher whose path never changes (mise/asdf: `<root>/shims/node`; volta:
// `<home>/.volta/bin/node`). Prefer it when the interpreter is under a manager AND the launcher really
// runs; otherwise keep `process.execPath`, and the existing "node interpreter GONE" heal still covers it.
function defaultVerifyNode(node) {
  try { execFileSync(node, ['--version'], { stdio: 'ignore', timeout: 5000 }); return true; } catch { return false; }
}
export function resolveStableNode(exec = process.execPath, verify = defaultVerifyNode) {
  const parts = exec.split(path.sep);
  const base = path.basename(exec);
  const candidates = [];
  for (const marker of ['installs', 'versions']) {            // mise, asdf, nvm layouts
    const i = parts.indexOf(marker);
    if (i > 0) candidates.push(path.join(parts.slice(0, i).join(path.sep), 'shims', base));
  }
  const v = parts.indexOf('.volta');                          // volta: <home>/.volta/bin/node
  if (v > 0) candidates.push(path.join(parts.slice(0, v + 1).join(path.sep), 'bin', base));
  for (const shim of candidates) {
    try { if (shim !== exec && fs.existsSync(shim) && verify(shim)) return { node: shim, stable: true, from: exec }; } catch { /* try next */ }
  }
  return { node: exec, stable: false, from: exec };
}

export function plistBody(secs, node = resolveStableNode().node) {
  return `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>Label</key><string>${MONITOR_LABEL}</string>
  <key>ProgramArguments</key>
  <array>
    <string>${node}</string>
    <string>${MONITOR_SCRIPT}</string>
  </array>
  <key>StartInterval</key><integer>${secs}</integer>
  <key>RunAtLoad</key><true/>
  <!-- ProcessType=Standard, NOT Background (ADR-021). Background opts the job into macOS's most
       aggressive power management — App Nap, timer coalescing, deferral — which for a watchdog that must
       fire on schedule is exactly wrong, and is the configurable factor the drop evidence pointed at. The
       tick is a few stats every interval, so normal scheduling costs nothing. LowPriorityIO is dropped
       for the same reason: do not advertise this job as deferrable. -->
  <key>ProcessType</key><string>Standard</string>
  <key>StandardOutPath</key><string>${MONITOR_STDERR}</string>
  <key>StandardErrorPath</key><string>${MONITOR_STDERR}</string>
</dict>
</plist>
`;
}

// Returns { ok, err } — the REASON, not just the verdict. It used to swallow stderr and return
// a bare boolean, so a failed `launchctl load` reached monitorCommand as { ok: false } with no
// `why`; it printed the literal string `[monitor] undefined`, exited 0, and `make install` went
// on to announce "monitor scheduled". The monitor was not scheduled. A watchdog that fails to
// install, and says `undefined` while doing it, is the worst possible version of this bug.
// A test must NEVER touch the real launchd/cron. The LABEL is a module constant, NOT sandboxed by
// HOME_BASE — so a test that sandboxes HOME and calls `uninstallMonitor()` (MI4 does) or `installMonitor()`
// still runs `launchctl bootout gui/$UID/<real label>`, which boots out the DEVELOPER'S real monitor (or a
// user's, if they run `npm test` from a clone). That is the actual cause the drop hunt found, after
// wrongly suspecting power management. RSP_NO_LAUNCHCTL makes every launchctl/crontab MUTATION a no-op;
// the plist/meta/heartbeat file logic still runs, so the tests keep their coverage. Set by run-tests.sh.
const launchctlDisabled = () => process.env.RSP_NO_LAUNCHCTL === '1';

function launchctl(...args) {
  if (launchctlDisabled()) return { ok: true, err: '' };
  try {
    execFileSync('launchctl', args, { stdio: ['ignore', 'ignore', 'pipe'] });
    return { ok: true, err: '' };
  } catch (e) {
    const err = (e.stderr ? e.stderr.toString() : '').trim() || e.message || 'unknown error';
    return { ok: false, err };
  }
}

// The one source of truth for "is the job REALLY registered." `launchctl load`/`bootstrap`
// can exit 0 (or fail with "already bootstrapped" while it IS live), so the exit code is not
// trustworthy — ask launchd directly. `list` enumerates every loaded service; `print` is the
// fallback for hosts where `list` is restricted.
function isLaunchdLoaded() {
  if (launchctlDisabled()) return true; // tests do not touch real launchd; treat install as registered
  try {
    if (execFileSync('launchctl', ['list'], { encoding: 'utf8' }).includes(MONITOR_LABEL)) return true;
  } catch { /* fall through to print */ }
  return launchctl('print', `gui/${process.getuid()}/${MONITOR_LABEL}`).ok;
}

function readCrontab() {
  try { return execFileSync('crontab', ['-l'], { encoding: 'utf8' }); } catch { return ''; }
}
function writeCrontab(text) {
  if (launchctlDisabled()) return; // tests must not rewrite the real crontab (the cron half of the hazard)
  execFileSync('crontab', ['-'], { input: text.endsWith('\n') ? text : `${text}\n` });
}
export function cronWithout(text) {
  return text.split('\n').filter((l) => !l.includes(CRON_TAG)).join('\n').replace(/\n+$/, '');
}

// Write down exactly what we scheduled, so the prompt hook can verify it later with plain
// fs checks — no launchctl, no crontab, nothing that costs a subprocess on the prompt path.
function writeMonitorMeta(secs, node = process.execPath) {
  try {
    fs.mkdirSync(STABLE_DIR, { recursive: true });
    fs.writeFileSync(MONITOR_META_PATH, `${JSON.stringify({
      node,
      script: MONITOR_SCRIPT,
      intervalSec: secs,
      // The launchd label, so the liveness check can authoritatively ask the scheduler "are you loaded?"
      // WITHOUT importing this module (which would cycle through health.mjs). ADR-021.
      label: MONITOR_LABEL,
      scheduledAt: new Date().toISOString(),
    }, null, 2)}\n`);
  } catch { /* best-effort */ }
}

export function installMonitor() {
  const secs = interval();
  if (!fs.existsSync(MONITOR_SCRIPT)) {
    return { ok: false, how: 'none', why: `runtime not installed — run a patch target's \`install\` first (missing ${MONITOR_SCRIPT})` };
  }
  // Resolve the interpreter ONCE and use the same value for the plist/cron entry AND the recorded
  // meta, so the liveness check verifies the same node the scheduler runs (ADR-021).
  const { node } = resolveStableNode();
  writeMonitorMeta(secs, node);

  if (process.platform === 'darwin') {
    fs.mkdirSync(path.dirname(PLIST), { recursive: true });
    fs.writeFileSync(PLIST, plistBody(secs, node));

    // Modern launchctl API (`bootout`/`bootstrap`), NOT the deprecated `load`/`unload`. On
    // current macOS `load` is a legacy shim that can exit 0 WITHOUT registering the job — so a
    // rewritten plist silently ends up present-on-disk but not loaded while monitor.json still
    // records "scheduled". `bootstrap` fails loudly; and either way we VERIFY the label is live
    // before claiming success, because a watchdog that reports itself scheduled when launchd has
    // no such job is the exact failure this monitor exists to prevent.
    const domain = `gui/${process.getuid()}`;
    launchctl('bootout', `${domain}/${MONITOR_LABEL}`); // drop any previous definition (best-effort)
    launchctl('unload', PLIST);                          // legacy fallback for very old macOS (best-effort)
    let r = launchctl('bootstrap', domain, PLIST);
    // Pre-bootstrap macOS (< 10.11) has no `bootstrap` — fall back to `load` only there.
    if (!r.ok && /unrecognized|unknown|usage:/i.test(r.err)) {
      r = launchctl('load', PLIST);
    }
    if (!isLaunchdLoaded()) {
      return {
        ok: false,
        how: 'launchd',
        secs,
        where: PLIST,
        why: `agent did not register with launchd${r.ok ? '' : `: ${r.err}`}\n    the monitor is NOT scheduled — the patches will not be re-applied between sessions.\n    retry:  launchctl bootstrap ${domain} ${PLIST}`,
      };
    }
    return { ok: true, how: 'launchd', secs, where: PLIST };
  }

  if (process.platform === 'linux') {
    // cron granularity is 1 minute; anything finer is pointless here.
    const mins = Math.max(1, Math.round(secs / 60));
    const spec = mins === 1 ? '* * * * *' : `*/${mins} * * * *`;
    const line = `${spec} "${node}" "${MONITOR_SCRIPT}" >>"${MONITOR_STDERR}" 2>&1 ${CRON_TAG}`;
    const next = `${cronWithout(readCrontab())}\n${line}`.replace(/^\n/, '');
    try { writeCrontab(next); } catch (e) { return { ok: false, how: 'cron', why: e.message }; }
    return { ok: true, how: 'cron', secs: mins * 60, where: 'crontab' };
  }

  return {
    ok: false,
    how: 'manual',
    why: `unsupported platform (${process.platform}) — schedule this yourself every ${secs}s:\n    ${process.execPath} ${MONITOR_SCRIPT}`,
  };
}

export function uninstallMonitor() {
  // Drop the meta + heartbeat too, or the prompt hook would go on warning that a monitor we
  // deliberately removed "is not running".
  try { fs.rmSync(MONITOR_META_PATH, { force: true }); } catch { /* ignore */ }
  try { fs.rmSync(HEARTBEAT_PATH, { force: true }); } catch { /* ignore */ }

  if (process.platform === 'darwin') {
    const existed = fs.existsSync(PLIST);
    launchctl('bootout', `gui/${process.getuid()}/${MONITOR_LABEL}`); // modern API
    launchctl('unload', PLIST);                                        // legacy fallback (best-effort)
    if (existed) fs.rmSync(PLIST, { force: true });
    return { removed: existed, how: 'launchd', where: PLIST };
  }
  if (process.platform === 'linux') {
    const cur = readCrontab();
    if (!cur.includes(CRON_TAG)) return { removed: false, how: 'cron' };
    try { writeCrontab(cronWithout(cur)); } catch { return { removed: false, how: 'cron' }; }
    return { removed: true, how: 'cron' };
  }
  return { removed: false, how: 'manual' };
}

// The scheduled job records an ABSOLUTE node path (process.execPath). Version managers
// pin that per version — e.g. mise: .../installs/node/24.14.1/bin/node — so upgrading node
// deletes the interpreter out from under the job. launchd/cron keep reporting the job as
// "scheduled" while every run fails silently, which is the worst possible failure for a
// watchdog. Surface it instead of trusting the schedule.
export function recordedNode() {
  try {
    if (process.platform === 'darwin') {
      const m = fs.readFileSync(PLIST, 'utf8').match(/<string>([^<]*node)<\/string>/);
      return m ? m[1] : null;
    }
    const m = readCrontab().split('\n').find((l) => l.includes(CRON_TAG))?.match(/"([^"]+)"/);
    return m ? m[1] : null;
  } catch { return null; }
}

// Same failure mode as a vanished node interpreter, one level up: the scheduler records
// an ABSOLUTE path to the job script, so MOVING that script (flat -> lib/cwd/) leaves
// launchd/cron happily invoking a path that no longer exists — or, worse, an OLD copy
// still sitting there, so the monitor keeps "succeeding" while running stale code that
// doesn't know about half the installed targets. Detect it, and heal it on install.
export function recordedScript() {
  try {
    if (process.platform === 'darwin') {
      const m = fs.readFileSync(PLIST, 'utf8').match(/<string>([^<]*\.mjs)<\/string>/);
      return m ? m[1] : null;
    }
    const m = readCrontab().split('\n').find((l) => l.includes(CRON_TAG))?.match(/"([^"]+\.mjs)"/);
    return m ? m[1] : null;
  } catch { return null; }
}

/**
 * Re-register the schedule when it is broken in any of the three ways a watchdog dies
 * silently: (1) launchd no longer has the job — a rewritten plist that never registered, a
 * logout/bootout; (2) the recorded node interpreter is gone — a version-manager upgrade
 * deleted it out from under the job; (3) the job script path moved. installMonitor() rewrites
 * the plist with the CURRENT node + script and re-bootstraps, so one call heals all three.
 * Called from the SessionStart hook path (applyInstalled) and every patch `install`, so a
 * dropped agent self-heals next session instead of nagging on every prompt for hours.
 */
export function healMonitor() {
  // Never installed → nothing to heal (don't resurrect a monitor the user deliberately removed).
  let meta;
  try { meta = JSON.parse(fs.readFileSync(MONITOR_META_PATH, 'utf8')); } catch { return { healed: false }; }
  if (!meta?.script) return { healed: false };

  const sched = monitorScheduled();
  const node = recordedNode();
  const nodeGone = node ? !fs.existsSync(node) : false;
  const script = recordedScript();
  const scriptMoved = !!script && script !== MONITOR_SCRIPT;

  if (sched.scheduled && !nodeGone && !scriptMoved) return { healed: false };

  const reason = !sched.scheduled ? 'not-loaded' : nodeGone ? 'node-interpreter-gone' : 'script-moved';
  const r = installMonitor();
  return { healed: !!r.ok, reason, from: script, to: MONITOR_SCRIPT, why: r.ok ? undefined : r.why };
}

/**
 * UNCONDITIONALLY re-register the schedule. This is the prompt-hook recovery path (ADR-021), and it
 * differs from healMonitor() in one way that matters: healMonitor no-ops while launchd still reports the
 * job "scheduled", but the failure we actually see is "scheduled yet not ticking" — a throttled or zombie
 * job whose heartbeat has gone stale. Only the caller knows the job is not running (from the cheap
 * heartbeat check), so when it calls here we always bootout+bootstrap. It runs from the LIVE prompt hook,
 * never from the launchd job itself, so it is never a self-bootout. Returns the outcome to announce.
 */
export function recoverMonitor() {
  let meta;
  try { meta = JSON.parse(fs.readFileSync(MONITOR_META_PATH, 'utf8')); } catch { return { attempted: false, recovered: false, reason: 'not-installed' }; }
  if (!meta?.script) return { attempted: false, recovered: false, reason: 'not-installed' };
  const r = installMonitor();
  return { attempted: true, recovered: !!r.ok, why: r.ok ? undefined : r.why };
}

/** The crash launchd/cron captured for a job that died before it could log — for instrumentation. */
export function readMonitorStderr(max = 800) {
  try {
    const s = fs.readFileSync(MONITOR_STDERR, 'utf8').trim();
    return s ? s.slice(-max) : '';
  } catch { return ''; }
}

/** Drop the captured-crash file once we have surfaced it, so an old crash is not re-announced forever. */
export function clearMonitorStderr() {
  try { fs.rmSync(MONITOR_STDERR, { force: true }); } catch { /* best-effort */ }
}

export function monitorScheduled() {
  const node = recordedNode();
  const nodeOk = node ? fs.existsSync(node) : true;
  const script = recordedScript();
  const stale = node && !nodeOk
    ? `scheduled node interpreter is GONE (${node}) — every run fails silently; re-run \`monitor install\``
    : (script && script !== MONITOR_SCRIPT
      ? `scheduled job points at a STALE script (${script}) — it is not the current one; re-run \`monitor install\``
      : null);

  if (process.platform === 'darwin') {
    if (!fs.existsSync(PLIST)) return { scheduled: false, how: 'launchd' };
    let loaded = false;
    try { loaded = execFileSync('launchctl', ['list'], { encoding: 'utf8' }).includes(MONITOR_LABEL); } catch { /* ignore */ }
    return { scheduled: loaded, how: 'launchd', where: PLIST, node, stale };
  }
  if (process.platform === 'linux') {
    return { scheduled: readCrontab().includes(CRON_TAG), how: 'cron', where: 'crontab', node, stale };
  }
  return { scheduled: false, how: 'manual' };
}

export function lastRun() {
  try {
    const lines = fs.readFileSync(MONITOR_LOG, 'utf8').trim().split('\n').filter(Boolean);
    return lines.length ? lines[lines.length - 1] : null;
  } catch { return null; }
}
