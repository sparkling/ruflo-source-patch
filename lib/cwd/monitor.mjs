...
export function runOnce() {
  ...
  // includes(), not startsWith(): applyPlugins() prefixes its lines with the target name.
  for (const l of [...cli.log, ...plug.log].filter((l) => /\b(skip:|INCOMPLETE)/.test(l))) {
    appendLog(`WARN ${l}`);
  }
  ...
  recordProblems([...cli.log, ...plug.log].filter((l) => /\b(skip:|INCOMPLETE|re-baselined)/.test(l)));

  const repaired = cli.patched + plug.patched;
  return { skipped: false, repaired, unchanged: cli.unchanged + plug.unchanged, result: { cli, plug } };
}
