...
//   memory  .swarm/memory.db durability — cross-process write lock (#2621),
//           WAL-coherent reads (#2584 follow-ups), an integrity gate that refuses a
...
export const PATCH_TARGETS = ['cwd', 'daemon', 'memory', 'init'];
...
export const TARGET_INFO = {
  ...
  memory: 'memory.db durability — write lock (#2621) + WAL-coherent reads (#2584) + integrity gate (refuse a torn-image flush) + stale-writer guard (kills every pre-patch writer, daemon AND MCP client, to force fresh code; loudly warns since killing an MCP client needs a manual /mcp reconnect after) (ADR-023)',
  init: '`ruflo init` stops generating what the plugins already provide — the standalone `claude-flow` .mcp.json server AND the skills/commands/agents bundle (helpers kept). The durable complement to `plugin-only`. Plugin-always deployments only (#2640/#2685)',
};
...
// exists to fix ruflo #2621 ("50 acked, 25 on disk") and was only ever checked for textual presence.
...
try {
  ...
  const e = new Error('[ruflo-source-patch] REFUSING to write ' + p + ': the database is structurally damaged (' + why + '). A whole-file write here would overwrite the damage as if the store were empty and ACK the loss silently. Nothing was written; the file on disk is unchanged. Recover it first — inspect the sibling .swarm/backups/, or let ruflo rebuild it on next initialize (it recovers a torn DB automatically).');
...
function __rufloIntegrityCheck(p) {
  ...
  try { nfs = __rufloReq('fs'); } catch { return; }
  let size;
  try { size = nfs.statSync(p).size; } catch { return; } // no file yet: a legitimate first write
  ...
  try {
    ...
    /* any other error (permission, race, unusual fs): never block a legitimate write */
  }
}`,
  },
...
const LOCK_WRAP_SRC = `
{
  ...
  const __rufloGuard = (fn, check) => async function (arg) {
    const p = __rufloPathOf(arg);
    const h = await __rufloLockAcquire(p);
    try { if (check) __rufloIntegrityCheck(p); return await fn.call(this, arg); }
    finally { __rufloLockRelease(p, h); }
  };
  // WRITE mutators get the integrity gate (ADR-023): a whole-file flush over a torn
  // image destroys it silently, so refuse rather than overwrite. ensureSchemaColumns is
  // the init/repair path — it must be allowed to run ON a fresh or half-built database,
  // so it takes the lock but NOT the gate (gating it would block the very recovery that
  // heals a torn DB).
  storeEntry = __rufloGuard(storeEntry, true);
  getEntry = __rufloGuard(getEntry, true);
  deleteEntry = __rufloGuard(deleteEntry, true);
  applyTemporalDecay = __rufloGuard(applyTemporalDecay, true);
  ensureSchemaColumns = __rufloGuard(ensureSchemaColumns, false);
}
...
// Exported for the SUITE. daemon/spawn-lock-legacy targets a legacy fork (@sparkleideas/cli) that no
// longer exists on any machine here, so there is no real fixture to patch — the suite synthesizes one
// FROM THIS TABLE, so the fixture can never drift from the anchor it is meant to exercise.
export const ENTRIES = [
  ...
  {
    id: 'state/mcp-tools-hooks-tools',
    target: 'cwd',
  ...
  // plugin-duplicated artifacts in the first place. Two files, one target.
  //
  ...
  //     skills at 8K each. The host agent then enumerates all of them: Codex reports "Skill
  //     descriptions were shortened to fit the 2% skills context budget", so the project's OWN
  //     skills get degraded to make room for a copy of a repo. Upstream's own comment concedes it
  //     "clones the whole ruvnet/ruflo repo (~50MB)".
  //
  //     Same category as (1) and (2), which is why it belongs to `init`: the installed plugins
  //     already carry these skills, so the import is duplication that costs context budget.
  //
  ...
  //
  //     Anchored on the FIRST guard inside the try, not the function signature: the signature is
  //     one token from a rename, while this line is load-bearing behaviour. Neutralising the guard
  //     leaves the rest of the function intact and unreachable, exactly as (1)/(2) do.
  {
    target: 'init',
    ...
    edits: [
      {
        ...
        replace: "        if (true) // ruflo-source-patch(init): `skills add ruvnet/ruflo --skill ruflo` lands the WHOLE repo in .agents/skills/ruflo (97MB, 384 SKILL.md vs 4 real skills) and blows the host agent's skills context budget; also an unpinned npx fetch-and-execute of an undeclared dep (#2777)\n            return;\n        if (ctx.flags['no-skills-sh'] === true)\n            return;",
      },
    ],
  },
];
...
function entryApplied(src, entry) {
  ...
  const edApplied = (e) => src.includes(e.replace) || (e.optional && !src.includes(e.find));
