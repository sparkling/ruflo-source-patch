...
// Write only when the bytes actually change. The monitor re-applies on a timer, and a
// blind writeFileSync every tick would churn mtimes and risk rewriting a file while a
// live process is reading it — for no benefit.
...
export function apply(targets = []) {
  ...
  const result = { patched: 0, reverted: 0, skipped: 0, unchanged: 0, log: [] };
