...
//           whole-file flush over an already-torn image, and a stale-writer guard that restarts
//           positively identified pre-patch daemons. MCP clients are reported but never killed:
//           only their owning host can reconnect the stdio transport safely (ADR-023).
//
...
import { memoryMcpPathEntry } from './memory-mcp-path.mjs';
...
import {
  AGENT_MODEL_ANCHOR,
  AGENT_MODEL_REPLACEMENT,
  FALLBACK_MODEL_ANCHOR,
  FALLBACK_MODEL_REPLACEMENT,
  ROUTE_DESCRIPTION_ANCHOR,
  ROUTE_DESCRIPTION_REPLACEMENT,
  ROUTER_MODEL_ANCHOR,
  ROUTER_MODEL_REPLACEMENT,
  nativeAgentContract,
  nativeHooksContract,
} from '../ruflo-model-contract/patcher.mjs';

import { LEARNING_STATS_ENTRIES } from '../ruflo-learning-stats/patcher.mjs';
import { MEMORY_STATS_ENTRIES } from '../ruflo-memory-stats/patcher.mjs';
import { CONTEXT_ENTRIES } from '../ruflo-context-contract/patcher.mjs';

const MARKER = PATCH_MARKER;

export const PATCH_TARGETS = ['cwd', 'daemon', 'memory', 'init', 'plugin-hosts', 'ruflo-model-contract', 'ruflo-learning-stats', 'ruflo-memory-stats', 'ruflo-context-contract'];

export const TARGET_INFO = {
  'ruflo-context-contract': 'validated context-synthesis episodes; preserve actual outcomes, never invent rewards from facts (#3314)',
  'ruflo-memory-stats': 'complete statistics through the existing AgentDB connection; no raw WAL snapshot (#3311)',
  'ruflo-learning-stats': 'store-scoped learning counters, actual neural-store path and singular pattern coverage (#2245)',
  ...
  memory: 'memory.db durability above native #2878 — path-keyed native bridge identity (#3143), stronger fail-closed lock ownership, WAL-sidecar refusal (#2735), integrity gate, and positively-resolved stale-writer recovery (ADR-023)',
  ...
  'ruflo-model-contract': 'exact native model IDs plus an explicit caller-owned routing-tier boundary (#3215)',
};
...
try {
  ...
  // #3143: memory-bridge accepts an explicit dbPath but upstream caches one
  // ControllerRegistry for the entire process. In a long-lived MCP process the first
  // opened store therefore wins silently: project -> user hides user memory, while
  // user -> project can return user rows for a project-scoped request. Key every
  // promise/instance/availability/failure state by canonical database identity.
  //
  // Initialisation remains serial because upstream temporarily replaces the global
  // console.log while ControllerRegistry starts. Parallelising two first opens would
  // interleave those replacements and can leave the wrong logger installed. Once
  // initialised, registries are independent and all operations remain concurrent.
  bridgeRegistryMap: {
    deps: ['req'],
    src: `const __RSP_MEMORY_BRIDGE_PATHS_REVISION = "2026-08-31.2";
const __rufloBridgeStates = new Map();
let __rufloBridgeInitTail = Promise.resolve();
let __rufloLastBridgeFailureReason = null;
function __rufloBridgeKey(dbPath) {
  let resolved;
  let npath;
  try {
    const raw = typeof dbPath === 'string' && dbPath
      ? dbPath
      : typeof getAgentDbPath === 'function'
        ? getAgentDbPath()
        : getDbPath();
    if (raw === ':memory:') return raw;
    npath = __rufloReq('node:path');
    resolved = npath.resolve(raw);
  } catch {
    resolved = String(dbPath || '');
  }
  try {
    const nfs = __rufloReq('node:fs');
    const realpath = nfs.realpathSync.native || nfs.realpathSync;
    return realpath(resolved);
  } catch {
    try {
      const nfs = __rufloReq('node:fs');
      const realpath = nfs.realpathSync.native || nfs.realpathSync;
      const parent = realpath(npath.dirname(resolved));
      return npath.join(parent, npath.basename(resolved));
    } catch { /* unresolved parent: retain the absolute lexical identity */ }
    return resolved;
  }
}
function __rufloBridgeState(dbPath) {
  const key = __rufloBridgeKey(dbPath);
  let state = __rufloBridgeStates.get(key);
  if (!state) {
    state = { key, promise: null, instance: null, available: null, failureReason: null };
    __rufloBridgeStates.set(key, state);
  }
  return state;
}
function __rufloSerialiseBridgeInit(fn) {
  const run = __rufloBridgeInitTail.then(fn, fn);
  __rufloBridgeInitTail = run.then(() => undefined, () => undefined);
  return run;
}
function __rufloSetBridgeFailure(dbPath, reason) {
  const state = __rufloBridgeState(dbPath);
  state.failureReason = reason;
  __rufloLastBridgeFailureReason = reason;
  return state;
}`,
  },
...
export const ENTRIES = [
  memoryMcpPathEntry,
  ...CONTEXT_ENTRIES,
  ...LEARNING_STATS_ENTRIES,
  ...MEMORY_STATS_ENTRIES,
  // ── target: ruflo-model-contract ──────────────────────────────────────────
  // This belongs in the CLI composition engine, not plugin-compose: hooks-tools.js
  // is already shared with cwd, and both edits must rebuild from one pristine.
  {
    id: 'ruflo-model-contract/agent-schema',
    target: 'ruflo-model-contract',
    suffix: ['@claude-flow', 'cli', 'dist', 'src', 'mcp-tools', 'agent-tools.js'],
    nativeSatisfied: nativeAgentContract,
    edits: [{ find: AGENT_MODEL_ANCHOR, replace: AGENT_MODEL_REPLACEMENT }],
  },
  {
    id: 'ruflo-model-contract/routing-tier',
    target: 'ruflo-model-contract',
    suffix: ['@claude-flow', 'cli', 'dist', 'src', 'mcp-tools', 'hooks-tools.js'],
    nativeSatisfied: nativeHooksContract,
    edits: [
      { find: ROUTE_DESCRIPTION_ANCHOR, replace: ROUTE_DESCRIPTION_REPLACEMENT },
      { find: FALLBACK_MODEL_ANCHOR, replace: FALLBACK_MODEL_REPLACEMENT },
      { find: ROUTER_MODEL_ANCHOR, replace: ROUTER_MODEL_REPLACEMENT },
    ],
  },
  // ── target: cwd ────────────────────────────────────────────────────────────
  {
  ...
  {
    id: 'memory/path-keyed-bridge',
    target: 'memory',
    suffix: ['@claude-flow', 'cli', 'dist', 'src', 'memory', 'memory-bridge.js'],
    // 3.32.9 has the same three globals but not the later failure-reason API.
    // Its separate legacy entry owns that shape; do not report a false drift.
    appliesWhen: 'let bridgeFailureReason = null;',
    excludesWhen: 'const registryInstances = new Map();',
    frags: ['bridgeRegistryMap'],
    proof: 'const __RSP_MEMORY_BRIDGE_PATHS_REVISION = "2026-08-31.2";',
    edits: [
      {
        find: 'let registryPromise = null;\nlet registryInstance = null;\nlet bridgeAvailable = null;',
        replace: '// ruflo-source-patch (#3143): registry lifecycle is keyed by canonical database path.',
      },
      {
        find: `/**
...
let bridgeFailureReason = null;`,
        replace: `/**
 * Bridge availability and failure diagnostics are scoped to the canonical
 * database identity above. One failed store must not demote every other store
 * in this long-lived process to the whole-image fallback.
 */`,
      },
      {
        find: ' * Lazily initialize the ControllerRegistry singleton.',
        replace: ' * Lazily initialize the ControllerRegistry for one canonical database.',
      },
      {
        find: 'async function getRegistry(dbPath) {',
        replace: 'async function getRegistry(dbPath) {\n    const state = __rufloBridgeState(dbPath);',
      },
      {
        find: `        bridgeFailureReason = process.platform === 'win32'
        ...
        return null;`,
        replace: `        const reason = process.platform === 'win32'
            ? 'AgentDB native bridge disabled on Windows after #3024; set CLAUDE_FLOW_ENABLE_NATIVE_BRIDGE_ON_WINDOWS=1 to opt in'
            : 'AgentDB native bridge disabled by CLAUDE_FLOW_DISABLE_BRIDGE=1';
        state.available = false;
        __rufloSetBridgeFailure(dbPath, reason);
        return null;`,
        optional: true,
      },
      {
        find: `    if (bridgeAvailable === false)
        return null;
    if (registryInstance)
        return registryInstance;
    if (!registryPromise) {
        registryPromise = (async () => {`,
        replace: `    if (state.available === false)
        return null;
    if (state.instance)
        return state.instance;
    if (!state.promise) {
        state.promise = __rufloSerialiseBridgeInit(async () => {`,
      },
      {
        find: '                        dbPath: dbPath || getAgentDbPath(),',
        replace: '                        dbPath: state.key,',
      },
      {
        find: `                registryInstance = registry;
                bridgeAvailable = true;
                bridgeFailureReason = null;
                return registry;`,
        replace: `                state.instance = registry;
                state.available = true;
                __rufloSetBridgeFailure(dbPath, null);
                return registry;`,
      },
      {
        find: `                bridgeFailureReason = err instanceof Error ? err.message : String(err);
                bridgeAvailable = false;
                registryPromise = null;
                return null;`,
        replace: `                __rufloSetBridgeFailure(dbPath, err instanceof Error ? err.message : String(err));
                state.available = false;
                state.promise = null;
                return null;`,
      },
      {
        find: `                // Record WHY. This latches for the process lifetime (see the
                // bridgeFailureReason doc comment), so discarding the error here
                // makes the resulting sql.js-fallback refusal undiagnosable.`,
        replace: `                // Record WHY for this database only. Discarding it here makes
                // the resulting sql.js-fallback refusal undiagnosable.`,
        optional: true,
      },
      {
        find: `        })();
    ...
    return state.promise;
      ...
      {
        find: `        bridgeFailureReason = null;
        // #2775: strict insert against an ACTIVE existing row`,
        replace: `        __rufloSetBridgeFailure(options.dbPath, null);
        // #2775: strict insert against an ACTIVE existing row`,
      },
      {
        find: `        bridgeFailureReason = msg;
        return null;
    }
}
/**
 * Search entries via AgentDB v3.`,
        replace: `        __rufloSetBridgeFailure(options.dbPath, msg);
        return null;
    }
}
/**
 * Search entries via AgentDB v3.`,
      ...
      {
        find: `export async function isBridgeAvailable(dbPath) {
    if (bridgeAvailable !== null)
        return bridgeAvailable;
    const registry = await getRegistry(dbPath);
    return registry !== null;
}`,
        replace: `export async function isBridgeAvailable(dbPath) {
    const state = __rufloBridgeState(dbPath);
    ...
    return registry !== null;
      ...
      {
        find: `export function getBridgeFailureReason() {
    return bridgeFailureReason;
}`,
        replace: `export function getBridgeFailureReason(dbPath) {
    if (dbPath !== undefined)
        return __rufloBridgeState(dbPath).failureReason;
    return __rufloLastBridgeFailureReason;
}`,
      },
      {
        find: `export function __setMemoryBridgeRegistryForTests(registry) {
    registryInstance = registry;
    registryPromise = registry ? Promise.resolve(registry) : null;
    bridgeAvailable = registry ? true : null;
    bridgeFailureReason = null;
}`,
        replace: `export function __setMemoryBridgeRegistryForTests(registry, dbPath) {
    const state = __rufloBridgeState(dbPath);
    state.instance = registry;
    state.promise = registry ? Promise.resolve(registry) : null;
    state.available = registry ? true : null;
    state.failureReason = null;
    __rufloLastBridgeFailureReason = null;
}`,
      },
      {
        find: ' * Install a pre-initialized registry for deterministic bridge tests.',
        replace: ' * Install a pre-initialized registry for one deterministic database test.',
      },
      {
        find: ` * The cached state is cleared unconditionally. Previously the reset lived
 ...
 * therefore had no recovery path short of a restart.`,
        replace: ` * A supplied path clears only that canonical database. With no path, every
 * unique live registry is closed exactly once and all path states are cleared.`,
      },
      {
        find: `export async function shutdownBridge() {
    if (registryInstance) {
        try {
    ...
    bridgeFailureReason = null;
        ...
        replace: `export async function shutdownBridge(dbPath) {
    const selected = dbPath === undefined
        ? [...__rufloBridgeStates.entries()]
        : [[__rufloBridgeKey(dbPath), __rufloBridgeStates.get(__rufloBridgeKey(dbPath))]];
    const closed = new Set();
    for (const [key, state] of selected) {
        if (!state)
            continue;
        let registry = state.instance;
        if (!registry && state.promise) {
            try { registry = await state.promise; }
            catch { registry = null; }
        }
        if (registry && !closed.has(registry)) {
            closed.add(registry);
            try { await registry.shutdown(); }
            catch { /* Best-effort */ }
        }
        __rufloBridgeStates.delete(key);
    }
    __rufloLastBridgeFailureReason = null;
}`,
      },
  ...
  // Ruflo 3.41.2+ keys registry instances/promises by path.resolve() natively (#3196) but
  // keeps bridgeAvailable/bridgeFailureReason as one process-wide latch: one failed init
  // still demotes every other database to the sql.js fallback, and a symlink alias is a
  // second registry. Same canonical state map as above; only the anchors differ.
  {
    id: 'memory/path-keyed-bridge-maps',
    target: 'memory',
    suffix: ['@claude-flow', 'cli', 'dist', 'src', 'memory', 'memory-bridge.js'],
    appliesWhen: 'const registryInstances = new Map();',
    frags: ['bridgeRegistryMap'],
    proof: 'const __RSP_MEMORY_BRIDGE_PATHS_REVISION = "2026-08-31.2";',
    edits: [
      {
        find: 'const registryPromises = new Map();\nconst registryInstances = new Map();',
        replace: '// ruflo-source-patch (#3143): registry lifecycle is keyed by canonical database path.',
      },
      {
        find: 'let testRegistryOverride = null;\nlet bridgeAvailable = null;',
        replace: 'let testRegistryOverride = null;',
      },
      {
        find: `/**
 * Why the bridge is unavailable, when it is.
 ...
 */`,
    ...
    if (!registryPromise) {
        ...
        replace: `    if (state.available === false)
        return null;
    // The canonical identity (realpath; a symlink alias is the same database)
    // is resolved once by __rufloBridgeState and is what AgentDB opens.
    const resolvedPath = state.key;
    if (state.instance)
        return state.instance;
    if (!state.promise) {
        state.promise = __rufloSerialiseBridgeInit(async () => {`,
      ...
      {
        find: `                registryInstances.set(resolvedPath, registry);
                bridgeAvailable = true;
                bridgeFailureReason = null;
                return registry;`,
        replace: `                state.instance = registry;
    ...
    return state.promise;
      ...
      {
        find: `export function _resetRegistryCacheForTest() {
    ...
    bridgeFailureReason = null;
        ...
        replace: `export function _resetRegistryCacheForTest() {
    __rufloBridgeStates.clear();
    testRegistryOverride = null;
    __rufloLastBridgeFailureReason = null;
}`,
      },
 ...
 * Search entries via AgentDB v3.`,
    ...
    bridgeFailureReason = null;
        ...
        replace: `export function __setMemoryBridgeRegistryForTests(registry) {
    __rufloBridgeStates.clear();
    testRegistryOverride = registry;
    __rufloLastBridgeFailureReason = null;
}`,
 ...
 * unique live registry is closed exactly once and all path states are cleared.`,
  ...
  {
    id: 'memory/path-keyed-bridge-legacy',
    target: 'memory',
    suffix: ['@claude-flow', 'cli', 'dist', 'src', 'memory', 'memory-bridge.js'],
    appliesWhen: `                registryInstance = registry;
                bridgeAvailable = true;
                return registry;`,
    frags: ['bridgeRegistryMap'],
    proof: 'const __RSP_MEMORY_BRIDGE_PATHS_REVISION = "2026-08-31.2";',
    ...
    if (!state.promise) {
      ...
      {
        find: `                bridgeAvailable = false;
                registryPromise = null;`,
        replace: `                __rufloSetBridgeFailure(dbPath, 'ControllerRegistry initialization failed');
                state.available = false;
                state.promise = null;`,
      },
      {
        find: `                __rufloSetBridgeFailure(dbPath, 'ControllerRegistry initialization failed');
                state.available = false;
                state.promise = null;
        ...
        replace: `                __rufloSetBridgeFailure(dbPath, err instanceof Error ? err.message : String(err));
                state.available = false;
                state.promise = null;
                logDemotionOnce('getRegistry (path-scoped until shutdown)', err instanceof Error ? err.message : String(err));`,
        optional: true,
      },
      {
        find: `        })();
  ...
  {
    id: 'memory/path-keyed-bridge-diagnostics',
    target: 'memory',
    suffix: ['@claude-flow', 'cli', 'dist', 'src', 'memory', 'memory-initializer.js'],
    appliesWhen: 'async function walRefusalError(operation) {',
    edits: [
      {
        find: `async function walRefusalError(operation) {
    const base = 'memory database has an active native WAL connection '`,
        replace: `async function walRefusalError(operation, dbPath) {
    const base = 'memory database has an active native WAL connection '`,
      },
      {
        find: '        const reason = bridge?.getBridgeFailureReason?.();',
        replace: '        const reason = bridge?.getBridgeFailureReason?.(dbPath);',
      },
      {
        find: "error: await walRefusalError('write'),",
        replace: "error: await walRefusalError('write', dbPath),",
        all: true,
      },
      {
        find: "error: await walRefusalError('read/write'),",
        replace: "error: await walRefusalError('read/write', dbPath),",
      },
    ],
  },
  {
  ...
  {
    id: 'cwd/agentdb-vector-storage-path',
    target: 'cwd',
    suffix: ['agentdb', 'dist', 'src', 'core', 'AgentDB.js'],
    frags: [],
    edits: [
      {
        find: "import * as path from 'path';\nimport { fileURLToPath } from 'url';",
        replace: "import * as path from 'path';\nimport os from 'node:os';\nimport { randomUUID } from 'node:crypto';\nimport { fileURLToPath } from 'url';",
      },
      {
        find: "        const vectorDimension = this.config.vectorDimension || 384;\n        // Initialize database with unified fallback system",
        replace: [
      ...
      {
        find: "            dimensions: vectorDimension,\n            metric: 'cosine'",
        replace: "            dimensions: vectorDimension,\n            metric: 'cosine',\n            storagePath: vectorStoragePath",
      },
    ],
  },
  {
    id: 'cwd/agentdb-ruvector-storage',
    target: 'cwd',
    suffix: ['agentdb', 'dist', 'src', 'backends', 'ruvector', 'RuVectorBackend.js'],
    frags: [],
    edits: [
      {
...
function entryApplied(src, entry) {
  ...
  if (entry.excludesWhen && src.includes(entry.excludesWhen)) return false;
...
function installedPackageFile(file) {
    ...
    try {
      ...
      if (typeof meta.name === 'string' && typeof meta.version === 'string') {
        return {
          ...
          relative: path.relative(cursor, file),
        };
      }
...
function cleanSiblingPristine(file) {
  const current = fs.readFileSync(file, 'utf8');
  if (current.length > 0 && !isOurs(current)) return current;

  const backup = backupOf(file);
  if (!fs.existsSync(backup)) return null;
  const saved = fs.readFileSync(backup, 'utf8');
  return saved.length > 0 && !isOurs(saved) ? saved : null;
}

// Recover a missing pristine only from an independently installed, byte-proven
// copy of the exact same package version and package-relative file. Name/version/path
// equality narrows the candidates; reproducing the orphaned current bytes by applying
// every recognized live entry is the authority. A merely similar sibling is refused.
function recoverPristineFromSibling(file, current, allEntries, peers, result) {
  const identity = installedPackageFile(file);
  if (!identity) return null;
  const applied = allEntries.filter((entry) => entryApplied(current, entry));
  if (!applied.length) return null;

  for (const peer of peers) {
    if (peer === file) continue;
    const peerIdentity = installedPackageFile(peer);
    if (!peerIdentity
      || peerIdentity.name !== identity.name
      || peerIdentity.version !== identity.version
      || peerIdentity.relative !== identity.relative) continue;

    let candidate;
    try { candidate = cleanSiblingPristine(peer); } catch { continue; }
    if (candidate === null) continue;

    const frags = applied.flatMap((entry) => entry.frags || []);
    let reproduced = composePrelude(frags) + '\n' + candidate;
    for (const entry of applied) reproduced = applyEdits(reproduced, entry);
    if (reproduced !== current) continue;

    writeIfChanged(backupOf(file), candidate);
    result.log.push(`recovered-pristine ${file} from ${peer} — exact ${identity.name}@${identity.version} composition proof`);
    return candidate;
  }
  return null;
}

function rebuild(file, entries, result, allEntries = entries, peers = [file]) {
  ...
  let pristine = readPristine(file, result);
  if (pristine === null) {
    pristine = recoverPristineFromSibling(file, current, allEntries, peers, result);
  }
  if (pristine === null) {
  ...
  const usable = entries.filter((e) => {
    ...
    if (e.excludesWhen && pristine.includes(e.excludesWhen)) return false;
...
export function apply(targets = []) {
  ...
  const peers = [...byFile.keys()];

  // v4.45 withdrawal: this file has no remaining patch entries, so ordinary
  // discovery would strand the removed #3315 code on upgraded installations.
  // Restore only our marked executable from its existing pristine backup.
  for (const file of discover(['@claude-flow', 'cli', 'dist', 'src', 'mcp-tools', 'agentdb-tools.js'])) {
    const source = fs.readFileSync(file, 'utf8');
    if (!source.includes('// ruflo-source-patch #3315: native availability is not coverage.')
      || !source.includes("const retrievalContract = 'retained-sql-khop-v1';")) continue;
    const backup = backupOf(file);
    if (!fs.existsSync(backup) || !fs.statSync(backup).size || isOurs(fs.readFileSync(backup, 'utf8'))) {
      result.errors++;
      result.log.push(`error ${file}: withdrawn graph retrieval requires its pristine backup; left untouched`);
      continue;
    }
    if (!byFile.has(file)) byFile.set(file, []);
  }
  ...
  for (const [file, entries] of byFile) {
    ...
    try {
      if (active.length) rebuild(file, active, result, entries, peers);
  ...
  for (const entry of ENTRIES) {
    for (const file of discover(entry.suffix)) {
      try {
        ...
        if (entry.excludesWhen && source.includes(entry.excludesWhen)) continue;
