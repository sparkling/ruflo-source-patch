...
//   daemon  daemon command identity — every direct start/stop/status/supervisor path
//           keys Ruflo's native lock/PID machinery to one project root (#2877; root model #2633).
...
export const TARGET_INFO = {
  ...
  daemon: 'daemon command identity — direct start/stop/status converge on one project root (#2877 / #2633)',
  ...
  'plugin-hosts': 'dual-host marketplace reconciliation + audited same-version refresh (#2854/#2870)',
};
...
const LOCK_WRAP_SRC = `
{
  ...
  if (typeof purgeNamespace === 'function') purgeNamespace = __rufloGuard(purgeNamespace, true);
}
...
export const ENTRIES = [
  ...
  // ── target: daemon ─────────────────────────────────────────────────────────
  // The `daemon start|stop|status` COMMAND in current @claude-flow/cli. Direct daemon
  // commands skip daemon-autostart, so the cwd target cannot cover this path. The gap
  // remains live in clean 3.33.0: commands/daemon.js anchors .claude-flow/, daemon.pid,
  ...
  // retained byte-for-byte; the obsolete local legacy-lock injection has been retired.
  //
    ...
    edits: [
      {
        ...
        replace: '    subcommands: [listCommand, searchCommand, installCommand, uninstallCommand, upgradeCommand, toggleCommand, infoCommand, createCommand, rateCommand, __rspHostInstallCommand, __rspHostUninstallCommand, __rspHostSyncCommand, __rspHostRefreshCommand],',
      },
      {
  ...
  for (const entry of ENTRIES) {
    for (const file of discover(entry.suffix)) {
      try {
        ...
        if (entryApplied(source, entry)) out[entry.target].patched++;
      } catch { /* unreadable */ }
    }
  }
  return out;
