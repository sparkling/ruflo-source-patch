...
//   memory  .swarm/memory.db durability — cross-process write lock (#2878),
...
export const TARGET_INFO = {
  ...
  memory: 'memory.db durability — fail-closed write lock (#2878) + WAL-sidecar refusal (#2735) + integrity gate (refuse a torn-image flush) + stale-writer guard (kills every pre-patch writer, daemon AND MCP client, to force fresh code; loudly warns since killing an MCP client needs a manual /mcp reconnect after) (ADR-023)',
...
// exists to fix ruflo #2878 (clean 3.33.0: 12 acked, 2 on disk) and used to be checked only as text.
...
async function __rufloLockAcquire(p, createParent = false) {
  ...
  if (createParent) {
    try { nfs.mkdirSync(__rufloReq('node:path').dirname(lockFile), { recursive: true }); }
    catch (e) {
      throw __rufloLockFailure(p, 'could not create the database directory: ' + (e && e.message ? e.message : String(e)));
    }
  }
...
async function __rufloWithLock(p, fn, createParent = false) {
  ...
  const h = await __rufloLockAcquire(p, createParent);
...
const LOCK_WRAP_SRC = `
{
  ...
  const __rufloGuard = (fn, before, createParent = false) => async function (arg) {
