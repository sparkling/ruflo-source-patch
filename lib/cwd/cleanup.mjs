...
export function planCleanup(startDir, { allDaemons = false } = {}) {
  ...
  const inTree = daemonPids()
    ...
    .map((d) => ({ ...d, age: pidAge(d.pid), atRoot: path.resolve(d.cwd) === root }));

  const rootDaemons = inTree.filter((d) => d.atRoot);
  const strayDaemons = inTree.filter((d) => !d.atRoot);

  // Keep the OLDEST root daemon (most established); everything else is a duplicate.
  const keep = allDaemons ? null : rootDaemons.slice().sort((a, b) => a.pid - b.pid)[0] || null;
