...
//
// So the invariant is provenance, not location: the stable copy must match THE IMMUTABLE PACKAGE
// it was synced from. We record that source at sync time. A global/package-cache upgrade rewrites
// the bytes at the same recorded path -> drift -> heal. A Git checkout is deliberately different:
// it is a development input, and following it on a timer would deploy uncommitted or merely
// untagged work into every live session. Explicit `install` may copy a checkout, but the monitor
// never follows later checkout edits automatically.
...
// A Git checkout is a mutable development source, even when it happens to be clean right now.
// A commit is not a release, and the next edit can occur between two monitor ticks. Detect the
// checkout structurally instead of running Git: worktrees use a `.git` file, ordinary clones use
// a directory, and package/cache installs have neither anywhere below their package boundary.
export function sourceIsMutableCheckout(src = syncedFrom()) {
  if (!src) return false;
  let dir = path.resolve(src);
  const boundary = path.parse(dir).root;
  while (dir !== boundary) {
...
export function stableLibSourceKind() {
  const src = syncedFrom();
  if (!src) return 'missing';
  if (sourceIsMutableCheckout(src)) return 'mutable-checkout';
  return 'immutable';
}
 ...
 *
 * Returns null when automatic comparison is deliberately unavailable: no manifest yet, the
 * recorded source is gone, or it is a mutable Git checkout. Callers use stableLibSourceKind() to
 * distinguish those states instead of misreporting all of them as npx-only usage.
 */
export function stableLibDrift() {
  const src = syncedFrom();
  if (!src) return null;
  // Never turn a working checkout into a five-minute deployment channel. An explicit install is
  // the review boundary for checkout bytes; tagged self-update remains the unattended path.
  if (sourceIsMutableCheckout(src)) return null;
...
/**
 * Re-copy from the recorded immutable source. An explicit install may copy a working tree, but a
 * scheduled process must not turn subsequent checkout edits into an unattended deployment.
 *
...
export function healStableLib() {
  ...
  if (sourceIsMutableCheckout(src)) return { healed: 0, skipped: 'mutable-checkout' };
