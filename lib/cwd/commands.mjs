    ...
    try {
      ...
      return {
        ...
        log: [updated
          ...
          : `all ${current} enabled Ruflo host plugin copy(ies) already match the refreshed marketplaces`],
      };
    } catch (error) {
...
function patchCommandUnlocked(targets, action) {
  ...
  if (action === 'install' || action === 'init') {
    ...
    report(t, applied);
    if (targets.includes('plugin-hosts') && !applied.errors) reportHostAutoUpdate(t);
...
export function monitorCommand(action) {
  ...
  if (action === 'install' || action === 'init') {
    ...
    if (state.patchTargets.includes('plugin-hosts')) {
      const applied = apply(state.patchTargets);
      report(t, applied);
      if (!applied.errors) reportHostAutoUpdate(t);
    }
    ...
    // Same stale-writer recovery the scheduled tick runs, so `monitor run` is the "restart them now"
    // path the SessionStart warning points to (ADR-023).
