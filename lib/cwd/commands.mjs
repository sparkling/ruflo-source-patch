...
export function syncStableCopy() {
  const trees = ['cwd', 'adr-template', 'adr-index'];
  for (const dir of trees) {
    const src = path.join(LIB_ROOT, dir);
    const dest = path.join(STABLE_LIB, dir);
    if (!fs.existsSync(src)) continue;
    fs.mkdirSync(dest, { recursive: true });
    for (const f of fs.readdirSync(src)) {
      if (f.endsWith('.mjs')) fs.copyFileSync(path.join(src, f), path.join(dest, f));
    }
  }
  // Files that sit at lib/ root (the plugin registry the above import).
  for (const f of fs.readdirSync(LIB_ROOT)) {
    if (f.endsWith('.mjs')) fs.copyFileSync(path.join(LIB_ROOT, f), path.join(STABLE_LIB, f));
  }
...
export function problemsIn(r) {
  // includes(), not startsWith(): applyPlugins() prefixes each line with its target
  // ("adr-index: INCOMPLETE …"), so anchoring at the start silently missed every plugin
  // problem — the exact class of bug this function exists to report.
  return r.log.filter((l) => /\b(INCOMPLETE|skip:|re-baselined)/.test(l));
}
