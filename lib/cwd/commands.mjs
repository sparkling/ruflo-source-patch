...
import { readState, addTargets, removeTargets, migrateLegacyState, setPaused } from './state.mjs';
...
export function patchCommand(targets, action) {
  ...
  if (action === 'install' || action === 'init') {
    ...
    // Re-apply the installed set (what the SessionStart hook does) and un-pause.
    const state = setPaused(false);
    report(t, apply(state.patchTargets));
    ...
    report(t, apply([]));
    setPaused(true);
    log(t, 'library reverted on disk; targets kept but re-applying is PAUSED');
    log(t, 'the monitor/hook will NOT put it back — run `patch` (or any `install`) to resume');
    ...
    if (state.paused) log(t, 'PAUSED:   reverted — hook/monitor will not re-apply until `patch`');
...
export function monitorCommand(action) {
  ...
  if (action === 'install' || action === 'init') {
    ...
    const r = runOnce();
    if (r.paused) log(t, 'PAUSED by `revert` — not re-applying; run `patch` to resume');
    else if (r.skipped) log(t, 'no patch targets installed — nothing to do');
    else log(t, r.repaired ? `repaired ${r.repaired} file(s)` : `steady state (${r.unchanged} file(s) already correct)`);
  } else if (action === 'check') {
    const d = checkDrift();
    for (const u of d.uncovered) log(t, `WARN ${u}`);
    if (d.paused) {
      log(t, 'paused by `revert` — library intentionally unpatched (not drift); `patch` to resume');
    } else if (d.drifting.length) {
...
export function reapply() {
  ...
  if (state.paused || !state.patchTargets.length) return;
