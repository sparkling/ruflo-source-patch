...
try {
  ...
  if (problems.length) {
    ...
    // A re-baseline is the one thing we cannot verify for ourselves, so it gets INSTRUCTIONS rather
    // than a warning. The reader of this text is usually an agent, and it can act on them.
