...
try {
  ...
  // The hook must not block session startup, but a fail-closed mutation lock (or unreadable desired
  // state) must not look like a successful re-apply. Persist and surface the same actionable signal
  // the ordinary incomplete-patch path produces.
  const problem = `error SessionStart re-apply failed: ${err?.message || err}`;
  try { recordProblems([problem]); } catch { /* stdout below is still the primary channel */ }
  console.log([
    '[ruflo-source-patch] ATTENTION — installed patches were not re-applied:',
    `  ${problem}`,
    '  Check:  npx github:sparkling/ruflo-source-patch monitor check',
  ].join('\n'));
}
...
// ─── THE STALE-WRITER DETECTOR (ADR-023) ─────────────────────────────────────
//
// The write lock and integrity gate live INSIDE the patched memory-initializer.js — they
// protect the store only for a process that actually LOADED that patched code. A ruflo MCP
// server or daemon that started before the memory patch, or that runs an npx cache copy the
// patch never touched, keeps writing memory.db the old unguarded way from memory. That is the
// stale image that tore semantic-product-mock. We cannot reach into it — only a restart can.
//
// Same split as the leak detector above: the hook WARNS, the monitor tick ACTS (kills, so the
// writer reloads patched). Warning here is the belt to the monitor's suspenders — a writer
// that appeared since the last tick is named the moment you open a session.
try {
  const { staleWriters, describeStaleWriters } = await import('./stale-writer.mjs');
  const stale = staleWriters();
  if (stale.length) console.log(describeStaleWriters(stale).join('\n'));
} catch { /* a detector must never break the start of a session */ }
