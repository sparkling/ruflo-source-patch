 ...
 *
 * The install is delegated to the package itself (`npx github:REPO#TAG monitor install`), because that is
...
export async function selfUpdate({ fetchJson, run } = {}) {
  ...
  try {
    ...
    exec(`github:${REPO}#${tag}`);
    return { updated: true, from: current, to: tag };
  } catch (err) {
...
function defaultRun(spec) {
  execFileSync('npx', ['-y', spec, 'monitor', 'install'], {
