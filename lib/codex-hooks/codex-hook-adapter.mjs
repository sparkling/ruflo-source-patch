...
// spawn_agent/apply_patch canonical payloads, SessionStart/UserPromptSubmit
// context JSON, "defer", SessionEnd's three-second budget, and Stop's
...
function translateSessionStart(stdout) {
  const context = stdout.trim();
  if (!context) return '';
  try {
    const parsed = JSON.parse(context);
    if (parsed?.hookSpecificOutput?.hookEventName === 'SessionStart') return stdout;
  } catch { /* Brain currently emits plain context */ }
  return JSON.stringify({
    hookSpecificOutput: {
...
function runShim(hookId, extra) {
  ...
  if (hookId === 'session-start') stdout = translateSessionStart(stdout || '');
