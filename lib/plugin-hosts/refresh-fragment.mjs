...
// same-version collisions. Disabled and non-user-scoped plugins are preserved exactly as they are.
async function __rspUpdateHostPlugins(raw) {
  ...
  for (const row of claudeState) {
    if (row?.scope === 'user' && typeof row.id === 'string' && row.id.endsWith('@ruflo')) ids.add(row.id);
  }
  ...
  for (const pluginId of selected) {
    ...
    if (!claudeBefore) entry.hosts.claude = { status: 'not-installed' };
    else if (claudeBefore.enabled !== true) {
      entry.hosts.claude = { status: 'preserved-disabled', version: claudeBefore.version };
    } else {
      try {
        ...
        if (compared.equal) {
          entry.hosts.claude = { status: 'current', version: claudeBefore.version };
        } else {
          if (claudeBefore.version === sourceVersion) {
            ...
            await __rspHostExec('claude', ['plugin', 'update', pluginId, '--scope', 'user']);
          }
          ...
          if (!current?.version || current.enabled !== true) {
            throw new Error('update exited 0 but an enabled user-scoped version is absent');
          }
        ...
        entry.hosts.claude = { status: 'failed', error: error.message };
      }
    }
