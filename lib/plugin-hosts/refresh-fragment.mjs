...
async function __rspPluginVersion(root) {
  const fs = await import('node:fs');
  const path = await import('node:path');
  ...
  throw new Error(`plugin manifest version is missing under ${root}`);
}

// Update every installed, enabled Ruflo plugin through the host that owns it. A normal version
// change uses Claude's native update command; Codex has no update command, so it refreshes the
// marketplace then remove/adds only a tree that differs. The tree comparison also catches #2870's
// same-version collisions. Disabled plugins are preserved exactly as they are. User scope is always
// active; project/local scopes are updated from their registered project directory when it still exists.
// An orphaned registry entry is not runnable and is reported without recreating or editing that project.
async function __rspUpdateHostPlugins(raw) {
  const requested = raw ? __rspHostPluginId(raw) : null;
  const result = {
    issue: __RSP_HOST_PLUGIN_ISSUE,
    operation: requested ? 'update' : 'update-all',
    ...(requested ? { pluginId: requested } : {}),
    success: false,
    entries: [],
  ...
  let codexState;
  try {
    await Promise.all([__rspClaudeMarketplace(), __rspCodexMarketplace()]);
    await Promise.all([
      __rspHostExec('claude', ['plugin', 'marketplace', 'update', 'ruflo']),
      __rspHostExec('codex', ['plugin', 'marketplace', 'upgrade', 'ruflo', '--json']),
    ]);
    [claudeMarketplace, claudeState, codexState] = await Promise.all([
  ...
  const ids = new Set();
  for (const row of claudeState) {
    if (typeof row?.id === 'string' && row.id.endsWith('@ruflo')) ids.add(row.id);
  }
  for (const row of codexState.installed) {
    if (row?.marketplaceName === 'ruflo' && typeof row.pluginId === 'string') ids.add(row.pluginId);
  }
  const selected = requested ? [requested] : [...ids].sort();
  if (requested && !ids.has(requested)) {
    result.entries.push({ pluginId: requested, hosts: { claude: { status: 'not-installed' }, codex: { status: 'not-installed' } } });
    result.success = true;
    return result;
  }
  ...
  for (const pluginId of selected) {
    ...
    const entry = { pluginId, hosts: {} };
    const claudeRows = claudeState.filter((row) => row?.id === pluginId);
    const codexBefore = __rspCodexInstalledRow(codexState, pluginId);

    if (!claudeRows.length) entry.hosts.claude = { status: 'not-installed' };
    for (const claudeBefore of claudeRows) {
      const scope = claudeBefore.scope;
      const projectPath = claudeBefore.projectPath;
      const hostKey = scope === 'user' ? 'claude' : `claude:${scope}:${projectPath || '(unknown)'}`;
      if (claudeBefore.enabled !== true) {
        entry.hosts[hostKey] = { status: 'preserved-disabled', version: claudeBefore.version };
        continue;
      }
      if (scope === 'managed') {
        entry.hosts[hostKey] = { status: 'preserved-managed', version: claudeBefore.version };
        continue;
      }
      if (!['user', 'project', 'local'].includes(scope)) {
        entry.hosts[hostKey] = { status: 'failed', error: `unsupported Claude scope: ${String(scope)}` };
        continue;
      }
      const scoped = scope !== 'user';
      if (scoped && (typeof projectPath !== 'string' || !path.isAbsolute(projectPath)
          || !fs.existsSync(projectPath) || !fs.lstatSync(projectPath).isDirectory())) {
        entry.hosts[hostKey] = {
          status: 'preserved-inactive-project', version: claudeBefore.version,
        };
        continue;
      }
      try {
        const sourceRoot = path.join(claudeMarketplace.installLocation, 'plugins', pluginName);
        const sourceVersion = await __rspPluginVersion(sourceRoot);
        const compared = await __rspComparePluginTrees(sourceRoot, claudeBefore.installPath);
        if (compared.equal) {
          entry.hosts[hostKey] = { status: 'current', version: claudeBefore.version };
        } else {
          const execOptions = scoped ? { cwd: projectPath } : {};
          if (claudeBefore.version === sourceVersion) {
            await __rspHostExec('claude', ['plugin', 'uninstall', pluginId, '--scope', scope], execOptions);
            await __rspHostExec('claude', ['plugin', 'install', pluginId, '--scope', scope], execOptions);
          } else {
            await __rspHostExec('claude', ['plugin', 'update', pluginId, '--scope', scope], execOptions);
          }
          claudeState = await __rspClaudePlugins();
          const current = claudeState.find((row) => row?.id === pluginId
            && row?.scope === scope
            && (!scoped || row?.projectPath === projectPath));
          if (!current?.version || current.enabled !== true) {
            throw new Error(`update exited 0 but the enabled ${scope}-scoped version is absent`);
          }
          const verified = await __rspComparePluginTrees(sourceRoot, current.installPath);
          if (!verified.equal) throw new Error(`cache differs from refreshed marketplace: ${verified.difference}`);
          entry.hosts[hostKey] = {
            status: current.version === claudeBefore.version ? 'refreshed' : 'updated',
            version: current.version,
          };
          result.restartRequired = true;
        }
      } catch (error) {
        entry.hosts[hostKey] = { status: 'failed', error: error.message };
      }
    }

    if (!codexBefore) entry.hosts.codex = { status: 'not-installed' };
    else if (codexBefore.enabled !== true) {
      entry.hosts.codex = { status: 'preserved-disabled', version: codexBefore.version };
    } else {
      try {
        const installedRoot = path.join(
          codexHome, 'plugins', 'cache', 'ruflo', pluginName, codexBefore.version,
        );
        const compared = await __rspComparePluginTrees(codexBefore.source?.path, installedRoot);
        if (compared.equal) {
          entry.hosts.codex = { status: 'current', version: codexBefore.version };
        } else {
          ...
          if (!current?.version || current.enabled !== true) {
            throw new Error('update exited 0 but an enabled version is absent');
          }
          const verified = await __rspComparePluginTrees(
            current.source?.path,
            path.join(codexHome, 'plugins', 'cache', 'ruflo', pluginName, current.version),
          );
          if (!verified.equal) throw new Error(`cache differs from refreshed marketplace: ${verified.difference}`);
          entry.hosts.codex = {
            status: current.version === codexBefore.version ? 'refreshed' : 'updated',
            version: current.version,
          };
          result.restartRequired = true;
        }
      } catch (error) {
        entry.hosts.codex = { status: 'failed', error: error.message };
      }
    }
    result.entries.push(entry);
  }

  result.success = result.entries.every((entry) =>
    Object.values(entry.hosts).every((host) => host.status !== 'failed'));
  return result;
}
...
export const HOST_REFRESH_FRAGMENT = [
  ...
  __rspPluginVersion.toString(),
  __rspUpdateHostPlugins.toString(),
