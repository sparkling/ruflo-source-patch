...
async function __rspResolveHostExecutable(command) {
  ...
  const home = typeof process.env.HOME === 'string' && path.isAbsolute(process.env.HOME)
    ? process.env.HOME
    : os.homedir();
  add(path.join(home, '.local', 'bin'));
  if (process.env.NPM_CONFIG_PREFIX) add(path.join(process.env.NPM_CONFIG_PREFIX, 'bin'));

  const npm = spawnSync('npm', ['prefix', '--global'], {
    encoding: 'utf8', timeout: 5000, maxBuffer: 1024 * 1024,
  });
  if (!npm.error && npm.status === 0) add(path.join(String(npm.stdout).trim(), 'bin'));
...
async function __rspHostExec(command, args, options = {}) {
  const { spawnSync } = await import('node:child_process');
  const windows = process.platform === 'win32';
  const executable = windows
    ? (process.env.ComSpec || 'cmd.exe')
    : await __rspResolveHostExecutable(command);
