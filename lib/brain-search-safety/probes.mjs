...
export function probeSymbolRouteSource(source) {
  const stop = extract(source, 'const SYM_STOP =', source.includes('\nexport function symbolRoute')
...
export function safeSharedFailureGuidance(source) {
  const start = source.indexOf('export function describeSearchFailure({ dir }) {');
  const end = start < 0 ? -1 : source.indexOf('\n}', start);
  if (start < 0 || end < 0 || hasForbiddenRepair(source)) return false;
  try {
    const fn = source.slice(start, end + 2).replace('export function', 'function');
    const result = new vm.Script(`${fn}\ndescribeSearchFailure({ dir: '/safe/kb' })`)
      .runInNewContext({}, { timeout: 1000 });
    return typeof result === 'string'
      && result.includes('Diagnose first:')
      && result.includes('/safe/kb')
      && result.includes('Reinstall dependencies only after confirming')
      && result.includes('do not modify the installation based on this outage alone')
      && !hasForbiddenRepair(result);
  } catch { return false; }
}

export function safeMcpFailureGuidance(source, shared = '') {
  const common = source.includes('RUVNET BRAIN IS DOWN')
    && source.includes('First error: ${firstErr}')
    && source.includes('isError: true')
    && !hasForbiddenRepair(source);
  return common && (/do not[^\n]*(?:repair|mutate|install|download|reinstall)/i.test(source)
    || (source.includes("import { describeSearchOutcome, describeSearchFailure } from './search-outcome.mjs';")
      && source.includes('describeSearchFailure({ dir: KB_DIR })')
      && safeSharedFailureGuidance(shared)));
}

export function safeCliFailureGuidance(source, shared = '') {
  const common = source.includes("console.error('First error: ' + failed[0][1])")
    && source.includes('process.exit(1)')
    && !hasForbiddenRepair(source);
  return common && (/(?:no automatic repair|do not[^\n]*(?:repair|mutate|install|download|reinstall))/i.test(source)
    || (source.includes("import { describeSearchFailure } from './search-outcome.mjs';")
      && source.includes('console.error(describeSearchFailure({ dir: path.resolve(dir) }))')
      && safeSharedFailureGuidance(shared)));
}
