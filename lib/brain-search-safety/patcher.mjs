...
const satisfied = (source, kind) => {
  ...
  if (kind === 'mcp' || kind === 'cli') {
    try { shared = fs.readFileSync(path.join(kbRoot(), 'search-outcome.mjs'), 'utf8'); } catch { /* no native helper */ }
  }
  if (kind === 'router') return probeSymbolRouteSource(source).state === 'proven';
  if (kind === 'mcp') return safeMcpFailureGuidance(source, shared);
  if (kind === 'cli') return safeCliFailureGuidance(source, shared);
  return false;
};
