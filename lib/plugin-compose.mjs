...
const ORDER = [adrTemplate, adrIndex, verifyInterface, mcpPrefix];
...
const isOurs = (src) => ORDER.some((d) => d.isPatched(src));
...
export function applyComposed(installed = []) {
  ...
  for (const [file, claimants] of fileMap(active)) {
    try {
      ...
      const { pristine, rebaselined, empty, poisoned } = resolvePristine(file, composed, { isOurs });
