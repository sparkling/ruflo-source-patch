...
const ORDER = [
  adrTemplate, adrIndex, adrIoSafety, verifyInterface, rufloHooksSchema, mcpPrefix, designWall, flywheelDaily,
  brainConsoleLifecycle, brainConsoleProviderKeys, brainReleaseLockstep, brainMemoryDoctorRoots,
  brainSearchSafety, brainDualHostReceipt, brainDualHostStdin, brainGroundingEvidence,
...
export function composeSource(source, installed = []) {
  return ORDER
    .filter((descriptor) => installed.includes(descriptor.name))
  ...
  for (const [file, claimants] of fileMap(active)) {
    ...
    try {
      ...
      const recoverPoisoned = (current) => {
        const applied = claimants.filter((d) => (d.hasPatch || d.isPatched)(current));
      ...
      const selectedNames = claimants.map((descriptor) => descriptor.name);
      const selectedIsOurs = (src) => isOurs(src, selectedNames);
      ...
      for (const c of contribs) {
        if (c.missing.length && !c.applied.length && !BY_NAME[c.name].missingIsIncomplete) {
...
export function reconcile(installed = [], removed = []) {
  ...
  for (const [f, claimants] of fileMap(removedDescs)) {
    if (stillClaimed.has(f)) continue; // a surviving target owns it — applyComposed re-derived it

    const isKnownPatched = (pristine, current) => {
      // Build every subset of the descriptors being removed. Exact byte equality is the proof;
      // unrelated patchers are neither scanned nor accepted as historical provenance by inference.
      let candidates = new Set([pristine]);
      for (const d of claimants) {
        const next = new Set(candidates);
        for (const src of candidates) next.add(contribute(d, src).out);
        candidates = next;
      }
