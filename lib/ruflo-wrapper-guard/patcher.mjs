// Upstream ruvnet/ruflo#3306: refuse the branding wrapper, never select a driver.
...
export function patchSource(source) {
  ...
  if (FORMATS.some(format => source === format(REFUSAL_SOURCE))) return { next: source, applied: [], missing: [] };
  const format = FORMATS.find(format => source === format(WRAPPER_SOURCE));
  if (!format) {
    return { next: source, applied: [], missing: ['exact-published-wrapper-source'] };
  }
  return { next: format(REFUSAL_SOURCE), applied: ['refuse-branding-wrapper'], missing: [] };
}
...
export const isPatched = (source) => FORMATS.some(format => source === format(REFUSAL_SOURCE))
  || source.includes(HOOK_REPLACEMENT) || source.includes(LEGACY_REPLACEMENT);
export function reverseSource(source) {
  const format = FORMATS.find(format => source === format(REFUSAL_SOURCE));
