...
import {
  HOOK_ANCHOR, HOOK_ANCHOR_026, HOOK_REPLACEMENT, HOOK_REPLACEMENT_026, LEGACY_ANCHOR, LEGACY_REPLACEMENT, PRIOR_HOOK_REPLACEMENT, PRIOR_LEGACY_REPLACEMENT, patchHook,
} from './hooks.mjs';
...
// Recognize our previous rendition exactly, including backup-free upgrades.
export const PRIOR_RUNTIME_SOURCE = RUNTIME_SOURCE.replace(
  'Retry through npx -y ruflo@latest <arguments>; MCP: npx -y ruflo@latest mcp start. No global installation is required.',
  'Repair the installed implementation: npm install --global @claude-flow/cli@latest');
...
export function patchSource(source) {
  ...
  const format = FORMATS.find(format => [WRAPPER_SOURCE, REFUSAL_SOURCE, PRIOR_RUNTIME_SOURCE].some(old => source === format(old)));
  if (!format) {
...
export const isPatched = (source) => FORMATS.some(format => source === format(RUNTIME_SOURCE))
  || source.includes(HOOK_REPLACEMENT) || source.includes(HOOK_REPLACEMENT_026) || source.includes(LEGACY_REPLACEMENT);
export function reverseSource(source) {
  const format = FORMATS.find(format => [RUNTIME_SOURCE, PRIOR_RUNTIME_SOURCE, REFUSAL_SOURCE].some(patched => source === format(patched)));
  return format ? format(WRAPPER_SOURCE)
    : source.replace(HOOK_REPLACEMENT_026, HOOK_ANCHOR_026)
      .replace(HOOK_REPLACEMENT, HOOK_ANCHOR).replace(LEGACY_REPLACEMENT, LEGACY_ANCHOR)
      .replace(PRIOR_HOOK_REPLACEMENT, HOOK_ANCHOR).replace(PRIOR_LEGACY_REPLACEMENT, LEGACY_ANCHOR);
}
