...
function loopBody(source, start, end) {
  const from = source.indexOf(start);
  if (from < 0) return '';
  const to = source.indexOf(end, from + start.length);
  return to < 0 ? '' : source.slice(from, to);
}

function nativeStoreHelper(source) {
  return source.includes('#2660: pass --upsert explicitly')
    && source.includes('memoryStoreArgs(namespace, key, value)');
}

function nativeEdgeHelper(source) {
  return source.includes('const allEdges = uniqueEdges(parsedEdges);')
    && source.includes("memoryStore('adr-edges', edgeKey(e), edgeValue(e))");
}

function memoryCountReady(source) {
  const body = loopBody(source, 'function memoryCount(namespace) {', '\n}\n\nconst dryRun');
  return body.includes('memoryListComplete(namespace)')
    || (body.includes('cwd: ROOT')
      && (body.includes('CLI_PKG') || body.includes('memoryStoreArgs(namespace')));
}
...
export const EDITS = [
  ...
  {
    id: 'upsert',
    done: (src) => src.includes("'--upsert',") || nativeStoreHelper(src),
    variants: [
      {
  ...
  //    reappears it is a genuine failure and belongs in `errors`.
  {
    id: 'records-miscount',
      ...
      src,
      "const r = memoryStore('adr-patterns'",
      "const r = memoryStore('adr-edges'",
    ).includes("if (r === 'ok') storedRecords++;"),
    variants: [{
  ...
  // index ends at 4, desired is 3).
  {
    ...
    variants: [
      ...
      {
        find: "    return 'error: ' + (r.stderr || r.stdout || '').slice(0, 100);\n  }\n  return 'ok';\n}\n\nconst dryRun = process.env.IMPORT_DRY_RUN === '1';",
        replace: "    return 'error: ' + (r.stderr || r.stdout || '').slice(0, 100);\n  }\n  return 'ok';\n}\n\n// ruflo-source-patch (#2660): how many rows does this namespace actually hold?\n// --limit 1 keeps it cheap; the total is reported regardless of the limit. Returns null\n// when the total can't be read — never a guess, because a wrong count here would either\n// cry wolf or hide a real orphan.\nfunction memoryCount(namespace) {\n  const cliPackage = memoryStoreArgs(namespace, '', '')[0];\n  const r = spawnSync('npx', [\n    cliPackage, 'memory', 'list', '--namespace', namespace, '--limit', '1',\n  ], { stdio: ['ignore', 'pipe', 'pipe'], encoding: 'utf-8', cwd: ROOT });\n  const m = /Showing\\s+\\d+\\s+of\\s+(\\d+)\\s+entries/i.exec(`${r.stdout || ''}${r.stderr || ''}`);\n  return m ? parseInt(m[1], 10) : null;\n}\n\nconst dryRun = process.env.IMPORT_DRY_RUN === '1';",
      },
    ],
  },
  {
...
function hasPatch(src) {
  return src.includes(OWN_MARKER);
}
...
function applySelected(source, ids) {
  let next = source;
  for (const edit of EDITS) {
    if (!ids.has(edit.id) || edit.done(next)) continue;
    const variant = edit.variants.find((item) => occurrences(next, item.find) === 1);
    if (!variant) return null;
    next = next.replace(variant.find, variant.replace);
  }
  return next;
}

function marketplaceRecovery(current, file) {
  const root = path.join(HOME_BASE, '.claude', 'plugins', 'marketplaces', MARKETPLACE);
  const relative = path.relative(root, file);
  const expected = path.join('plugins', PLUGIN_NAME, ...SCRIPT_SUFFIX);
  if (relative !== expected) return null;

  const gitRoot = spawnSync('git', ['-C', root, 'rev-parse', '--show-toplevel'], {
    encoding: 'utf8', timeout: 10_000,
  });
  if (gitRoot.status !== 0) return null;
  try {
    if (fs.realpathSync(gitRoot.stdout.trim()) !== fs.realpathSync(root)) return null;
  } catch {
    return null;
  }

  const normalized = relative.split(path.sep).join('/');
  const shown = spawnSync('git', ['-C', root, 'show', `HEAD:${normalized}`], {
    encoding: 'utf8', timeout: 10_000, maxBuffer: 4 * 1024 * 1024,
  });
  if (shown.status !== 0 || !shown.stdout) return null;

  let candidates = [{ source: shown.stdout, ids: new Set() }];
  for (const edit of EDITS) {
    const expanded = [...candidates];
    for (const candidate of candidates) {
      if (edit.done(candidate.source)) continue;
      const ids = new Set(candidate.ids);
      ids.add(edit.id);
      const source = applySelected(candidate.source, ids);
      if (source !== null) expanded.push({ source, ids });
    }
    candidates = expanded;
  }
  const match = candidates.find((candidate) => candidate.source === current);
  if (!match) return null;
  return {
    candidate: shown.stdout,
    verify: (candidate) => applySelected(candidate, match.ids),
  };
}

// The composable descriptor. The per-file loop (pristine, compose, write, restore, reporting) lives in
// the shared engine (plugin-compose.mjs), so this target composes with mcp-prefix on a shared file from
// ONE pristine instead of fighting it over a backup (ADR-020). `patchSource`'s INCOMPLETE/ambiguous
// reporting is preserved verbatim — the engine renders it.
