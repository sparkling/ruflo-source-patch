...
// here (#2666 lacked `memory purge`; #2621 remained incomplete), so every unknown keeps the patch.
...
// Functional, CODE-level anchors from ruvnet-brain's replacements — not prose comments, which can be
// reworded without the underlying behaviour changing:
...
//   #48 implementation: the raw-Bash gate is now advisory-only and its executable output explicitly
//              says it never blocks; structured ruvnet_cli_help/run owns enforcement instead. That
//              stronger replacement has no override because there is no raw-Bash block to override.
...
//
// A marker is a LIST of accepted spellings; any one of them counts. That is not indulgence, it is the
// same lesson as the prose-vs-code distinction above, one level down: `"$NODE_BIN" -e '` was never the
// fix, it was one SPELLING of the fix. Upstream shipped the JSON parsing inline first and later
// extracted it verbatim into a shared, tested module (their ADR-0021, scripts/hook-input.mjs) — the
// behaviour did not change at all, only where it lives. Matching the inline form alone made this
// predicate answer `live` against a copy that was already fixed, so the target never retired and the
// SessionStart hook kept re-applying a patch whose anchors no longer matched anything. Anchor the
// BEHAVIOUR (the payload reaches a real JSON parser), and accept every spelling that delivers it.
...
/**
 * Does `src` satisfy a marker? ONE implementation, exported and shared by the predicate below and by
 * its fixture test — the same reason hook-input.mjs exists upstream. Two copies of "what counts as
 * fixed" is how the predicate and the test silently stop agreeing with each other.
 */
