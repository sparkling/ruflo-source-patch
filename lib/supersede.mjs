...
import { retireTarget as retireTargetInState } from './cwd/state.mjs';
 ...
 */
export function supersededAmong(installed = [], pinned = []) {
  const out = [];
  for (const t of installed) {
    if (!SUPERSEDED_BY[t]) continue;
    if (pinned.includes(t)) continue;      // the user has said keep it. That is final.
    const r = evaluate(t);
...
export function retireSuperseded(state) {
  ...
  for (const { target, evidence, issue, replacement } of supersededAmong(installed, state.pinned)) {
    try {
      ...
      out.log.push(
        ...
        + `re-apply it. This is not a failure: upstream now does this job. To get it back: `
        + `\`${target} unretire\`. To have kept it: \`${target} pin\`. (${issue})`,
      );
    } catch (err) {
