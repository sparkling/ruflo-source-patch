...
// incompatibilities:
//   1. hooks.json has unsupported top-level metadata (proposed in PR #2800, released by PR #2857); and
    ...
    for (const subcommand of ['modify-bash', 'modify-file']) {
      ...
      const run = spawnSync(process.execPath, [file, subcommand], {
        ...
        env: {
          ...
          PLUGIN_ROOT: path.dirname(path.dirname(file)),
        },
      });
      let calls = [];
...
export const rufloHooksSchemaSupersession = {
  ...
  replacement: 'native strict hook manifests (PR #2857) and Codex-compatible PreToolUse output (#2816)',
