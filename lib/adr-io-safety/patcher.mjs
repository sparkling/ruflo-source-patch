...
const nativeRuntimeCall = "spawnSync(MANAGED_CLI, ['memory', ...args, '--path=' + DB_PATH]";
const legacyRuntimeCall = "spawnSync('npx', [CLI_PKG, 'memory', ...args, '--path=' + DB_PATH]";
export const isPatched = (source) => hasPatch(source)
  && source.includes(nativeRuntimeCall)
  && behaviorShape(source);
const isLegacyPatched = (source) => hasPatch(source)
  && source.includes(legacyRuntimeCall)
  && behaviorShape(source);
  ...
  for (const root of roots) {
    for (const name of SCRIPT_NAMES) {
      ...
      try {
        ...
        const outcome = isLegacyPatched(source)
          ? { missing: [] }
          : patchSource(source);
