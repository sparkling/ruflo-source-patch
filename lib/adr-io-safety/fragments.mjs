...
function runMemory(args, inspectContent = true) {
  const result = spawnSync('npx', [CLI_PKG, 'memory', ...args, '--path=' + DB_PATH], {
...
if (process.env.CLI_CORE === '1') {
  console.warn('[ruflo-adr] warning: CLI_CORE=1 is ignored so verify reads the same managed store as import/reindex (#2781).');
}`;
