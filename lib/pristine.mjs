 ...
 *
 * `revert()` bypasses every guard in resolvePristine(), because it doesn't need a pristine:
...
export function restoreFromBackup(file) {
  ...
  if (!fs.existsSync(backup)) return { reverted: false };
  if (fs.statSync(backup).size === 0) {
    fs.rmSync(backup, { force: true });
    return { reverted: false, poisoned: true };
  }
 ...
 */
export function resolvePristine(file, patchFn) {
  const backup = backupOf(file);
  ...
  if (saved.length === 0) {
    fs.rmSync(backup, { force: true });
    const patchedNow = patchFn(current);
    if (patchedNow === current) {
      // The file is already ours and we have no usable pristine — no honest way to recover
      // the vendor original. Do not invent one.
  ...
  if (current === patchFn(saved)) return { pristine: saved, rebaselined: false }; // case 1
