...
export function resolvePristine(file, patchFn, { isOurs } = {}) {
  const backup = backupOf(file);
  ...
  if (!fs.existsSync(backup)) {
    ...
    // inventing one would bake the patch into the baseline. Refuse rather than guess.
    if (isOurs && isOurs(current)) return { pristine: null, rebaselined: false, poisoned: true };
