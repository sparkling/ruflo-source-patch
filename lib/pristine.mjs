...
/**
 * Restore a file from its backup — but only when the caller can prove the live bytes are one of
 * our known patch compositions of that backup.
 *
 * The UNINSTALL path bypasses every guard in resolvePristine(), because it doesn't need a pristine:
 * it used to copy the backup back unconditionally. That made TWO destructive cases possible:
 *
 *   1. an empty backup truncated the live file to zero bytes; and
 *   2. an in-place upstream update left a stale, non-empty backup beside NEW fixed bytes, and
 *      uninstall/retirement copied the OLD vendor file over the replacement.
 *
 * `isKnownPatched(pristine, current)` is the proof boundary. If it says false and the current file
 * does not carry this removed target's patch (`hasPatch(current)`), reality outranks the snapshot:
 * preserve current and discard the stale backup. If the current file DOES still carry the patch
 * but cannot be reproduced from the backup, preserve both and report `unresolved`; guessing either
 * way would lose bytes.
 */
export function restoreFromBackup(file, { isKnownPatched, hasPatch } = {}) {
  ...
  try {
    ...
    // Missing can mean an upstream deletion, but it is also a normal instant during an in-place
    // plugin extraction. Preserve any recovery bytes and keep desired state until a later tick can
    // classify the completed installation; absence is not proof that cleanup succeeded.
    return {
      restored: false,
      preserved: true,
      missingCurrent: true,
      unresolved: true,
      reason: `live file is missing${backupExists ? '; kept its pristine backup' : ' and no pristine backup exists'}`,
    };
  }
  const patched = typeof hasPatch === 'function' && hasPatch(current);
  ...
  if (!backupExists) {
    if (current.length === 0) {
      return {
        restored: false,
        unresolved: true,
        reason: 'live file is empty and no pristine backup exists',
      };
    }
    return patched
      ? { restored: false, unresolved: true, reason: 'patched bytes remain but the pristine backup is missing' }
      : { restored: false, alreadyClean: true };
  }

  if (fs.statSync(backup).size === 0) {
    fs.rmSync(backup, { force: true });
    return current.length === 0 || patched
      ? { restored: false, poisoned: true, unresolved: true, reason: 'live bytes or their pristine backup are empty; restoration cannot be proven' }
      : { restored: false, poisoned: true, preserved: true };
  }
  ...
  if (current.length === 0) {
    return {
      ...
      preserved: true,
      unresolved: true,
      reason: 'live file is empty; kept the valid pristine backup and refused to overwrite a possible in-progress update',
    };
  }
  if (current === saved) {
    fs.rmSync(backup, { force: true });
    return { restored: true, alreadyClean: true };
  }

  if (typeof isKnownPatched === 'function' && isKnownPatched(saved, current)) {
    try {
      writeIfChanged(file, saved, { expectedCurrent: current });
    } catch (err) {
      if (err?.code !== 'RSP_CONCURRENT_CHANGE') throw err;
      return {
        restored: false,
        preserved: true,
        unresolved: true,
        reason: 'live bytes changed after provenance proof; refused to overwrite a concurrent plugin update',
      };
    }
    fs.rmSync(backup, { force: true });
  ...
  if (patched) {
    return {
      ...
      preserved: true,
      unresolved: true,
      reason: 'live bytes carry the patch but are not a known composition of the pristine backup',
    };
  }

  // Neither pristine nor one of our derivations: an external writer replaced the file in place.
  // Preserve those bytes and remove the stale snapshot so a later uninstall cannot resurrect it.
  fs.rmSync(backup, { force: true });
  return { restored: false, preserved: true, staleBackupRemoved: true };
}

/** Atomic write, and only when the bytes actually differ. */
export function writeIfChanged(file, next, { expectedCurrent } = {}) {
  let current;
  let currentMode = null;
  try { current = fs.readFileSync(file, 'utf8'); } catch { current = null; }
  try { currentMode = fs.statSync(file).mode & 0o7777; } catch { /* new file */ }
  if (expectedCurrent !== undefined && current !== expectedCurrent) {
    const err = new Error(`concurrent change detected for ${file}`);
    err.code = 'RSP_CONCURRENT_CHANGE';
    throw err;
  }
  ...
  let fd;
  try {
    fd = fs.openSync(tmp, 'wx', currentMode ?? 0o666);
    fs.writeFileSync(fd, next);
    if (currentMode !== null) fs.fchmodSync(fd, currentMode);
    fs.fsyncSync(fd);
    fs.closeSync(fd);
    fd = undefined;
    if (expectedCurrent !== undefined) {
      let latest;
      try { latest = fs.readFileSync(file, 'utf8'); } catch { latest = null; }
...
export function resolvePristine(file, patchFn, { isOurs, recoverPoisoned } = {}) {
  ...
  if (!fs.existsSync(backup)) {
    ...
    if (isOurs && isOurs(current)) {
      ...
      if (recovered !== null) { writeIfChanged(backup, recovered); return { pristine: recovered, rebaselined: false, recovered: true }; }
  ...
  if (saved.length === 0) {
    ...
    writeIfChanged(backup, current); // current is clean vendor code — adopt it
    return { pristine: current, rebaselined: true };
  }

  // A non-empty backup can still be poisoned: an earlier composition bug may have adopted one of
  // our own outputs as the "vendor" baseline. Treat that exactly like a missing/empty pristine.
  // If live bytes are now clean upstream bytes, reality wins and becomes the new baseline. If both
  // carry our edits, accept only a caller-supplied reconstruction whose forward transform reproduces
  // the live file byte-for-byte.
  if (isOurs && isOurs(saved)) {
    if (!isOurs(current)) {
      writeIfChanged(backup, current, { expectedCurrent: saved });
      return { pristine: current, rebaselined: true };
    }
    const recovered = tryRecover();
    if (recovered !== null) {
      writeIfChanged(backup, recovered, { expectedCurrent: saved });
      return { pristine: recovered, rebaselined: false, recovered: true };
    }
    return { pristine: null, rebaselined: false, poisoned: true };
  }
