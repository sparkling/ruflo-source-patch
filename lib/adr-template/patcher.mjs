...
export function apply() {
  ...
  for (const file of discover()) {
    ...
    // Exactly one occurrence, or we do not touch it — see occurrences() above.
    const usable = EDITS.filter((e) => !e.done(pristine) && occurrences(pristine, e.find) === 1);
    const missing = EDITS.filter((e) => !e.done(pristine) && occurrences(pristine, e.find) !== 1);
