...
`adr-index`'s parser — its **sibling skill, in the same plugin** — only recognises an *unprefixed*
...
`isPatched()` was `!EDITS.some(e => src.includes(e.find))` — patched *iff the anchor is absent*. Upstream
re-indents one line, every anchor stops matching, and the file reads as **fixed**: `status` said `1/1
patched`, `monitor check` said `ok — no drift`, and #2659 was fully live.
