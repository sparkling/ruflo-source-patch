...
export function replaceOnce(src, before, after, label) {
  ...
  // A replacement function keeps Markdown/regex/shell payloads literal. Passing the payload
  // directly would let JavaScript expand $&, $`, $', and $$ replacement metacharacters.
...
export function isNativeBrainReady(command, src) {
  ...
  const installedVersion = body.includes('codex plugin list --available --json')
    ...
    && occurrences(body, 'codex plugin list --available --json') === 1;
  ...
  const exactTagNotes = body.includes('application/vnd.github.raw+json')
    ...
    && body.includes('`^[0-9]+\\.[0-9]+\\.[0-9]+(?:-[0-9A-Za-z.-]+)?$`, then retrieve only that exact tag with:');
  const mentionsExactTag = body.includes('application/vnd.github.raw+json')
    || body.includes('RELEASE-NOTES-4.0.md?ref=')
    || body.includes('Never use `latest`');
  const immutableExecutable = body.includes("installed plugin's `scripts/whats-new.mjs` executable")
    && body.includes('same immutable plugin payload')
    && body.includes('exits nonzero if either asset is missing')
    && body.includes('Do not substitute a checkout, download, or another installed version')
    && body.includes("Read that command's output before summarizing it")
    && body.includes('If the release notes are missing, say they are unavailable')
    && !body.includes('codex plugin list --available --json');
  return (installedVersion || immutableExecutable)
    && occurrences(body, '# What is new') === 1
    && (immutableExecutable || exactTagNotes || (installedNotes && !mentionsExactTag))
