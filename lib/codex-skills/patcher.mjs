...
const SPECS = {
  ...
  'brain-codex-skills': {
    ...
    plugin: 'ruvnet-brain',
    issue: 'stuinfla/ruvnet-brain#76',
    native: [{ command: 'whats-new', skill: 'whats-new' }],
    aliases: [],
  },
};
...
export function resolveActive(spec) {
...
function nativeEntries(root, spec) {
  return spec.native.map(({ command, skill }) => {
    ...
    assertNoSymlink(root, file);
    // Once a plugin ships a native skill, that file is the authority. Parsing its legacy Claude
    // command as well made an unrelated command-text refresh block an otherwise exact native edit.
    // The command is needed only to synthesize an additive skill for older plugin generations.
    const additiveBody = () => renderNativeAdditive(
      command, skill, commandSource(root, command), spec.issue,
    );
    const body = fs.existsSync(file) ? null : additiveBody();
    return {
      kind: 'native',
      command,
      file,
      ...
      if (entry.kind === 'native') {
        if (fs.existsSync(entry.file)) {
          ...
          if (owned && !fs.existsSync(backup)) {
            try {
              if (entry.additiveBody() === current) {
                out.unchanged++;
                continue;
              }
            } catch {
              // Fall through to the loud incomplete result: without a pristine backup or an exact
              // additive reconstruction, an owned-looking native file is not safe to overwrite.
            }
            out.incomplete++;
...
export function status(target) {
  ...
  for (const entry of entries) {
    try {
      const current = fs.readFileSync(entry.file, 'utf8');
      const additiveReady = entry.kind === 'native'
        && isOwned(current, spec.issue)
        && !fs.existsSync(backupOf(entry.file))
        && entry.additiveBody() === current;
