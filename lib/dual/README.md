...
An item is removed **only when a plugin actually provides it**, so anything project-unique is kept.
Everything removed is backed up first. `--strip-dup-hooks` is opt-in because it edits `settings.json`.
Start with `--dry-run`.
