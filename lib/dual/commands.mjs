...
export const SCRIPT_TARGETS = {
  ...
  'adr-reindex': {
    ...
    // The script hard-deletes rows from memory.db through raw sqlite3, and depends on the
    // `memory` target for BOTH halves of that being safe: memory/write-lock makes <db>.rsp-lock
    // mean something (a lock nothing else takes protects nothing), and memory/wal-coherent-reads
    // stops a reader acting on a stale pre-delete image. The script refuses to run without it —
    // say so HERE too, so the dependency is discovered at install time and not at the moment
    // someone actually needs a reconcile.
    requires: ['memory'],
  },
};
...
export function scriptCommand(targetKey, action) {
  ...
  if (action === 'install' || action === 'init') {
    // A declared prerequisite is checked, or it is decoration.
    const missingDeps = (spec.requires || []).filter((t) => !readState().patchTargets.includes(t));
    if (missingDeps.length) {
      log(`requires the ${missingDeps.join(', ')} patch target(s), which are not installed.`);
      log(`install them first:  ruflo-source-patch ${missingDeps.join(' install && ruflo-source-patch ')} install`);
      process.exitCode = 1;
      return true;
    }
