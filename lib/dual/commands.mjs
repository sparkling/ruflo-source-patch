...
//
//   ruflo-codex-hooks   repair an EXISTING Codex installation by registering
//                       Ruflo's canonical marketplace plugin. Ruflo v3.32.24+
//                       handles new `--codex`/`--dual` initializations upstream.
//                       -> ~/.ruflo-source-patch/ruflo-codex-hooks/
//
//   codex-switch        continue ONE Codex resume ID while changing which account
//                       pays for it: the subscription (`openai`) or a Copilot
//                       Responses proxy (`copilot`). Rewrites nothing but the
//                       provider-private encrypted replay items, under a lock,
//                       against a verified backup (ADR-030).
//                       -> ~/.ruflo-source-patch/codex-switch/
//
...
export const SCRIPT_TARGETS = {
  'dual-codex-claude': {
    dir: DUAL_DIR,
    files: ['ruflo-add-codex.sh', 'ruflo-new-dual.sh', 'ruflo-mcp-registration.mjs'],
    dirs: ['templates'],
  ...
  'dedupe-bundle': {
    ...
    usage: '<project-dir> [--keep-dup-hooks|--keep-dup-mcp|--keep-server|--bundle-only] [--dry-run]',
    blurb: 'make a project plugin-only: strip the duplicated .claude bundle, hooks, MCP registration + server (#2640)',
  },
  'ruflo-codex-hooks': {
    dir: RUFLO_CODEX_HOOKS_DIR,
    files: ['ruflo-codex-hooks.sh', 'ruflo-codex-hooks.mjs'],
    dirs: [],
    entry: 'ruflo-codex-hooks.sh',
    usage: '[--dry-run]',
    blurb: 'register Ruflo’s canonical Codex lifecycle plugin without touching MCP (#2801)',
  },
  'codex-switch': {
    dir: CODEX_SWITCH_DIR,
    files: ['codex-switch.sh', 'codex-switch.mjs', 'codex-switch-core.mjs', 'codex-switch-profile.toml'],
    dirs: [],
    entry: 'codex-switch.sh',
    usage: '<openai|copilot|status> [--session UUID] [--force-boundary] [--prepare-only] [--dry-run] [--safe] [-- CODEX_ARGS…]',
    blurb: 'continue one Codex resume ID across the subscription account and a Copilot proxy (ADR-030)',
  },
...
// v4.31.0 briefly materialized inferred `.codex/skills/*/skill.toml` manifests
// beside the dual scripts. That whole materialized directory was package-owned
// and is now obsolete. Remove that exact directory without following a symlink;
// project-local `.codex/skills` cleanup is separately marker-gated in the script.
function pruneMaterializedCodexSkills(root) {
  let stat;
  try {
    stat = fs.lstatSync(root);
  } catch (err) {
    if (err.code === 'ENOENT') return [];
    return [`codex-skills (${err.message})`];
  }
  if (stat.isSymbolicLink() || !stat.isDirectory()) {
    return ['codex-skills (refusing to remove a symlink or non-directory at the obsolete package path)'];
  }
  try {
...
function materialize(spec) {
  ...
  const failed = spec === SCRIPT_TARGETS['dual-codex-claude']
    ? pruneMaterializedCodexSkills(path.join(spec.dir, 'codex-skills'))
    : [];
