...
|---|---|---|
| `AGENTS.md` | the project's `AGENTS.md` | **the single source of truth** — every shared instruction. Codex reads it directly; Claude Code imports it. |
...
**The whole point is the asymmetry.** `CLAUDE.md` must stay thin. Anything that belongs to both platforms
goes in `AGENTS.md`, or it will drift — which is the bug the `dual` target exists to prevent
