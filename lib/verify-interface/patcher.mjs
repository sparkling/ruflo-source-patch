...
export function apply() {
  ...
  for (const file of discover()) {
    try {
      ...
      if (missing.length) {
        // A PARTIAL apply here is worse than none: the regex edit adds capture groups, so if it landed
        // and a BASH_REMATCH consumer did not, the gate reads the wrong groups and blocks on garbage.
        result.incomplete++;
        result.log.push(`INCOMPLETE ${file} — applied: ${applied.join(', ') || 'none'}; NOT APPLIED: ${missing.join(', ')} (the regex groups and their readers must move together — uninstall this target)`);
      } else if (changed) {
