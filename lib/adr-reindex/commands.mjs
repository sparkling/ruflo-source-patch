// Dispatch for the legacy `adr-reindex` target. It retires only when #2666's native
// skill, purge command, and shared ordinary-writer lock are all runnable here.
//
...
import {
  apply, restore, status, supersessionCheck,
} from './patcher.mjs';
...
// resurrect everything (ruvnet/ruflo#2878). The script refuses at run time too — this check exists so
...
export function adrReindexCommand(action) {
  ...
  if (action === 'install' || action === 'init') {
    ...
    if (missing.length) {
      ...
      log('resurrect every row it removes (ruvnet/ruflo#2878). Install it first:');
  ...
  if (action === 'status') {
    ...
    const retirement = state.retired['adr-reindex'];
    if (retirement) {
      ...
      const current = supersessionCheck();
      log(`current revalidation: ${current.state} — ${current.evidence}`);
      return true;
    }
    const installed = state.pluginTargets.includes('adr-reindex');
    const s = status();
