...
// `withMemoryDbLock()` uses <db>.lock only around purge, while this project's #2621 patch makes
// every writer use <db>.rsp-lock. The replacement is safe here only when purgeNamespace is wrapped
// in that same guard too; mere subcommand presence is not enough.
export function upstreamPurgeSharesWriteLock() {
  ...
  for (const root of roots) {
    ...
    if (!source.includes("const lockFile = p + '.rsp-lock';")
        || !source.includes('purgeNamespace = __rufloGuard(purgeNamespace, true);')) {
      return false;
    }
  }
  return seen;
...
export function supersessionCheck() {
  ...
  if (!upstreamPurgeSharesWriteLock()) {
    return {
      state: 'live',
      evidence: 'ruflo-adr and `memory purge` are present, but purge does not share <db>.rsp-lock '
        + 'with ordinary writers; #2666 point 1 is still open and a pre-delete image can be restored',
    };
  }
  return {
    state: 'superseded',
