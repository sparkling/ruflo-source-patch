...
// Current Ruflo ships a native skill and purge command, but purge's private <db>.lock is not shared by
// ordinary writers. This target retires only after the memory patch puts native purge under the same
// <db>.rsp-lock as those writers; older plugin/CLI pairs keep the additive command below.
//
...
// #2666 requires purge to take the SAME write lock as ordinary memory writers. Native
// `withMemoryDbLock()` uses <db>.lock only around purge, while this project's #2878 patch makes
// every writer use <db>.rsp-lock. Require the CURRENT fail-closed marker too: the older patch had
// a similar wrapper but proceeded unlocked when acquisition failed.
export function upstreamPurgeSharesWriteLock() {
  const SUFFIX = ['@claude-flow', 'cli', 'dist', 'src'];
  const roots = [...GLOBAL_ROOTS];
  try {
    for (const dir of fs.readdirSync(NPX_ROOT)) roots.push(path.join(NPX_ROOT, dir, 'node_modules'));
  } catch { /* no npx cache */ }
  ...
  for (const root of roots) {
    ...
    if (!source.includes("const lockFile = p + '.rsp-lock';")
...
export function supersessionCheck() {
  ...
  if (!upstreamPurgeSharesWriteLock()) {
    return {
      state: 'live',
      evidence: 'ruflo-adr and `memory purge` are present, but purge does not share the current '
        + 'fail-closed <db>.rsp-lock with ordinary writers; #2666 point 1 remains open',
    };
  }
  return {
    state: 'superseded',
    evidence: 'ruflo-adr ships its own /adr-reindex AND the installed @claude-flow/cli registers '
      + '`memory purge`, and purge shares the same <db>.rsp-lock as ordinary writers',
  };
}
      ...
      if (fs.existsSync(file)) {
        ...
        if (!isOurs(cur)) {
          ...
          result.log.push(sharedLock
            ? `skip:upstream-owns-it ${file} — ruflo-adr ships its own adr-reindex, the installed CLI has \`memory purge\`, and purge shares <db>.rsp-lock with ordinary writers. Theirs works here. Uninstall this target.`
            : purge
              ? `skip:upstream-owns-it ${file} — ruflo-adr ships its own adr-reindex and \`memory purge\`, but purge does NOT share the installed writers' <db>.rsp-lock. That leaves #2666 point 1 open. Leaving their file alone. OURS still reconciles under the shared lock: ${REINDEX_SCRIPT_HINT}. Do NOT uninstall this target yet.`
