...
function transformPlatformMarkdown(source, reverse = false, revision = 'current') {
  const crlf = source.includes('\r\n');
  let next = normalize(source);
  const applied = [];
  const missing = [];
  for (const [id, original, patched, previousPatched] of PLATFORM_REPLACEMENTS) {
    const target = reverse ? original
      : revision === 'previous' && previousPatched ? previousPatched : patched;
    if (occurrences(next, target) === 1) continue;
    const candidates = reverse
      ? [patched, previousPatched].filter(Boolean)
      : revision === 'previous' ? [original] : [previousPatched, original].filter(Boolean);
...
export function patchPlatformGenerator(source) {
  return patchPlatformGeneratorRevision(source, 'current');
}

function patchPlatformGeneratorRevision(source, revision) {
  const literal = platformLiteral(source);
  if (!literal) return { next: source, applied: [], missing: ['platform-generator-surface'] };
  const result = revision === 'previous'
    ? transformPlatformMarkdown(literal.markdown, false, 'previous')
    : patchPlatformSkill(literal.markdown);
...
export const patchPreviousPlatformGenerator = (source) => patchPlatformGeneratorRevision(source, 'previous');

export function recoverPlatformGenerator(source) {
  const literal = platformLiteral(source);
  if (!literal) return null;
  const revision = literal.markdown.includes(PLATFORM_MCP_PATCHED) ? 'current'
    : literal.markdown.includes(PLATFORM_MCP_PATCHED_V1) ? 'previous' : null;
  if (!revision) return null;
  const candidate = reversePlatformGenerator(source);
  if (candidate === source) return null;
  const verify = revision === 'current'
    ? (pristine) => patchPlatformGenerator(pristine).next
    : (pristine) => patchPreviousPlatformGenerator(pristine).next;
  return verify(candidate) === source ? { candidate, verify } : null;
}
