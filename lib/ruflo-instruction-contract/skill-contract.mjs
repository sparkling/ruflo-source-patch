...
export function recoverPreviousPlatformGenerator(source) {
  const literal = platformLiteral(source);
  if (!literal || !literal.markdown.includes(PLATFORM_MCP_PATCHED_V1)) return null;
  const candidate = reversePlatformGenerator(source);
  if (candidate === source) return null;
  const verify = (pristine) => patchPreviousPlatformGenerator(pristine).next;
