...
inspect the installed executable's help; never guess flags or claim a scan ran.`,
    required: ['aidefence_scan', 'ruvnet_cli_help', 'ruvnet_cli_run'],
  },
};
...
export function patchBuiltInSkill(source) {
  ...
  const { original, patched } = DEFINITIONS[name];
...
export function reverseBuiltInSkill(source) {
  ...
  const crlf = source.includes('\r\n');
  const next = normalize(source).replace(DEFINITIONS[name].patched, DEFINITIONS[name].original);
...
const PLATFORM_REPLACEMENTS = [
  ...
  ['platform-runtime', PLATFORM_MCP, PLATFORM_MCP_PATCHED],
...
function transformPlatformMarkdown(source, reverse = false) {
  ...
  const applied = [];
  const missing = [];
  for (const [id, original, patched] of PLATFORM_REPLACEMENTS) {
    const from = reverse ? patched : original;
    const to = reverse ? original : patched;
    if (occurrences(next, to) === 1) continue;
    const count = occurrences(next, from);
    if (count !== 1) missing.push(count > 1 ? `${id}(AMBIGUOUS: anchor occurs ${count}x)` : id);
    else { next = next.replace(from, to); applied.push(id); }
  }
...
export const isPlatformSkillPatched = (source) => normalize(source).includes(`${ISSUE_MARKER}:platform-skill`)
  && !normalize(source).includes('mcp__claude-flow__')
  && !normalize(source).includes('Full catalog: `npx ruflo mcp list`');
