...
export const SHARED_SECTIONS = {
  'Ruflo Interface Contract': `**ruflo-interface-contract:v2**
...
const KNOWN = {
  'Ruflo Interface Contract': new Set([
    ...
    'a97016455696df58a70bfb5d380a7edfd32653ed0b881015ee4668505d9a31b7', // patch v1 Markdown-marker wording
  ]),
...
const INTERFACE_V1_MARKER = '**ruflo-interface-contract:v1**';
const INTERFACE_V2_MARKER = '**ruflo-interface-contract:v2**';
const INTERFACE_V1_CLI = '- For a genuine CLI-only gap, use `ruvnet_cli_help`, then `ruvnet_cli_run` with literal `argv` when that bridge is registered. Exact requested help must authorize the run; parent help or exit code alone is insufficient.';
const INTERFACE_V2_CLI = '- For a genuine Ruflo CLI-only gap, use `ruvnet_cli_help({executable: "ruflo", argv: ["<group>", "<command>"]})`, then `ruvnet_cli_run({executable: "ruflo", argv: [...]})` with the exact literal arguments that help authorized. Never guess `claude-flow` as the executable merely because an old package or instruction used that name.';
const count = (source, needle) => source.split(needle).length - 1;

export function migrateInterfaceRevisionText(source) {
  const crlf = source.includes('\r\n');
  const normalized = normalize(source);
  const counts = {
    oldMarker: count(normalized, INTERFACE_V1_MARKER),
    newMarker: count(normalized, INTERFACE_V2_MARKER),
    oldCli: count(normalized, INTERFACE_V1_CLI),
    newCli: count(normalized, INTERFACE_V2_CLI),
  };
  if (counts.newMarker === 1 && counts.newCli === 1 && counts.oldMarker === 0 && counts.oldCli === 0) {
    return { next: source, changed: false };
  }
  if (counts.oldMarker !== 1 || counts.oldCli !== 1 || counts.newMarker !== 0 || counts.newCli !== 0) {
    return { error: `interface revision is edited, mixed, or ambiguous (${JSON.stringify(counts)})` };
  }
  const next = normalized.replace(INTERFACE_V1_MARKER, INTERFACE_V2_MARKER)
    .replace(INTERFACE_V1_CLI, INTERFACE_V2_CLI);
  return { next: crlf ? next.replaceAll('\n', '\r\n') : next, changed: true };
}
...
export function migrateInterfaceRevisionRoots(roots, { apply = false } = {}) {
  const plans = roots.map((root) => {
    ...
    const git = fs.lstatSync(path.join(resolved, '.git'));
    if (!git.isDirectory() || git.isSymbolicLink()) throw new Error(`root lacks a regular .git directory: ${root}`);
    const file = path.join(resolved, 'AGENTS.md');
    const stat = regular(file);
    const source = fs.readFileSync(file, 'utf8');
    const result = migrateInterfaceRevisionText(source);
    if (result.error) throw new Error(`${file}: ${result.error}`);
    return { file, source, next: result.next, stat, changed: result.changed };
  });
  const changed = plans.filter((item) => item.changed);
  if (apply) for (const item of changed) atomicWrite(item, item.next);
  return { roots: plans.length, files: plans.length, changed: changed.map((item) => item.file), applied: apply };
}
