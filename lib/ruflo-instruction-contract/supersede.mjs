...
import { discoverBundles, instructionContractViolations, PATCH_MARKER } from './patcher.mjs';
...
function check() {
  ...
  if (!claudeSurfaces || !codexSurfaces) {
    return {
      state: 'unknown',
      evidence: `native retirement needs both host surfaces overall; found Claude=${claudeSurfaces}, Codex=${codexSurfaces}`,
    };
  }
  return {
    state: 'superseded',
    evidence: `all ${bundles.length} installed generator bundle(s) render every present host template with the native structured-interface contract; both hosts are represented overall`,
  };
}
