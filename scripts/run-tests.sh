...
SUITES=(ruflo-wrapper-guard sequence-fuzz plugin-notify reporting hooks-migration untested concurrency memory-bridge-paths cwd-current daemon-native cleanup-procs stale-writer monitor-internals dual-host mcp-prefix adr-template-retirement adr-index-retirement adr-io-safety adr-reindex-native design-wall flywheel-daily codex-hooks codex-hooks-native codex-skills ruflo-codex-hooks codex-switch plugin-hosts-discovery plugin-hosts-codex-init plugin-hosts brain-release-lockstep brain-console-lifecycle brain-console-provider-keys brain-memory-doctor-roots brain-native-retirement brain-managed-memory-boundary brain-search-safety brain-dual-host-receipt brain-dual-host-stdin metaharness-codex-hooks ruflo-instruction-contract ruflo-instruction-migration ruflo-model-contract ruflo-learning-stats ruflo-learning-counter-writer)
SUITES+=(ruflo-memory-stats)
SUITES+=(ruflo-graph-persistence ruflo-context-contract)
SUITES+=(ruflo-graph-retrieval)
