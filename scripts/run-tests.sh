...
SUITES=(sequence-fuzz plugin-notify reporting untested concurrency cwd-current daemon-native cleanup-procs stale-writer monitor-internals dual-host mcp-prefix adr-template-retirement adr-index-retirement adr-reindex-native design-wall flywheel-daily codex-hooks codex-hooks-native codex-skills ruflo-codex-hooks plugin-hosts brain-release-lockstep brain-console-lifecycle brain-console-provider-keys brain-memory-doctor-roots brain-native-retirement brain-managed-memory-boundary metaharness-codex-hooks)
