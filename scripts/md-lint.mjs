...
// LINT WHAT WE WROTE, NOT WHAT WAS GENERATED INTO US.
//
// `ruflo init` writes ~196 markdown files into `.claude/` (bundled skills, commands and agents) and
// `codex init` another handful into `.agents/`. We did not author a line of them, they are regenerated on
// every init, and they duplicate the installed plugins by 97-100% (which is what `dedupe-bundle` exists to
// strip). Linting them turned a clean suite into 2878 problems across 235 files overnight, none of which
// anyone here could act on.
//
// A linter that reports thousands of failures in vendored files is a linter people switch off, and then it
// stops catching the one thing it is for. Scope is a correctness property, not a convenience.
const VENDORED = new Set([
  '.git', 'node_modules',
  '.claude', '.agents', '.codex',      // generated by `ruflo init` / `codex init`
